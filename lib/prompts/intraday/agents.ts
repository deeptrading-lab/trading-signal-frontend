...
export function formatIntradayContext(ctx: IntradayContext): string {
  ...
  return [
    ...
    // 수급 선행(I3) — 체결강도·호가 불균형. 미설정/빈값 시 빈 문자열(무주입).
    ...(ctx.orderFlowText ? [ctx.orderFlowText] : []),
    "",
...
[convictionScore 스케일 — 셋업 품질만 점수화]
- 이 점수는 **셋업 품질의 정직한 서술**입니다. 매수/매도 실행 여부·진입 문턱·비중은 서버 코드가
  별도 기준으로 판정하므로, 특정 점수를 "매수 문턱"으로 의식하거나 그 근처에서 멈추지 마세요.
  좋은 셋업은 주저 없이 높게, 애매한 셋업은 정직하게 중간으로 주세요.
- 50 = 중립(방향 근거 없음·상충). 50 초과 = 상승 확신, 50 미만 = 하락 확신.
- 90~100 강한 상승 확신(A+ 셋업 정렬 + 거래량 동반 + 수급 확인) / 75~89 뚜렷한 상승 우위(A+ 셋업 성립·거부권 없음) /
  60~74 약한 상승 기울기(방향 우위는 있으나 A+ 아님) / 45~59 중립·관망(근거 상충·거부권 공존) /
  30~44 약한 하락 기울기 / 15~29 뚜렷한 하락 우위 / 0~14 강한 하락 확신.
- 보유 중이면 점수는 "계속 들고 갈 확신"입니다 — 목표 도달·매물대 저항 부딪힘·흐름 둔화·논거
  훼손이면 점수를 확실히 낮추세요(낮은 점수는 청산 근거로 쓰입니다).

[★진입 우선순위 — 지표 평균이 아니라 "A+ 셋업 정렬" 여부로]
여러 지표를 평균내 뭉뚱그리지 마세요. **강한 매수 확신 구간(75↑)은 아래 A+ 셋업이 정렬되고 거부권이 없을 때만** 냅니다.
...
→ 위 셋업은 [수급 선행](체결강도·호가)이 **매수 우위로 확인**될 때 확신을 높이고, **매도 우위면 확인 실패**로 봅니다.
...
- [수급 선행] 체결강도·호가 매도 우위 — 특히 돌파·강세처럼 보이는데 실시간 수급이 매도 우위면 흡수/가짜 돌파 의심(단, 초단위 노이즈라 단독 근거로 삼지 말고 다른 근거와 결합).

[충돌 해소 — 엇갈리면 관망]
- A+ 셋업 정렬 + 거부권 없음 → 75~89(강한 정렬 + 거래량·수급 동반이면 90+).
- 트리거(진입 단서)와 거부권이 **함께** 있으면(예: 강세 RSI 다이버전스인데 VWAP 아래·위꼬리) → **거부권 우선, 관망(45~55)**. "절반은 좋고 절반은 나쁨"은 A+ 아님.
- 거부권만 있고 매수 근거 없음(하락 우위) → 낮은 점수(35 이하, 하락 확신).
- A+ 는 아니지만 방향 우위 근거가 명확히 하나 성립(거부권 없음) → 60~74(약한 상승 기울기). 근거가 하나뿐이라고
  중립으로 누르지 말고, A+ 와 구분되게 이 구간을 쓰세요.
