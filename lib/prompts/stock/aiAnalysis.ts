...
const LANG_INSTRUCTION =
  ...
  " 종목을 지칭할 때는 종목명(한글)만 사용하고, 6자리 종목 코드(숫자, 예: 017670)는 리포트 본문에 쓰지 마세요(종목 코드는 웹 검색·데이터 조회 용도로만 활용).";
...
const T = {
  NO_TOOL:   300_000,
  WEB_TOOL:  360_000,
  // PM = opus·effort:max + 최대 입력(11에이전트 종합 ~45k tok)·최대 출력이라 가장 오래 걸린다.
  //
  // ★상한은 **실측 분포를 따라 올려야 한다** — PM 프롬프트에 규칙이 추가될 때마다 소요시간이
  // 함께 늘기 때문. `ai_agent_usage` 실측(n=115):
  //   ~7/19  n=93 중앙 155s 최대 310s   ← 480초를 정할 때의 분포
  //   7/20~  n=22 중앙 261s 최대 405s   ← +68%. 480초까지 여유 75초(18%)뿐
  // 그 사이 PM 프롬프트에 무효화 라인(#350)·캘리브레이션 4규칙(#352)·가격 레벨 블록과
  // 재진입 앵커 규칙(#375)이 누적됐다. 실제로 최종 단계에서만 타임아웃 나는 실행이 반복
  // 관측됐고(한미약품 2회 등), **12분 걸린 분석 전체가 마지막 단계에서 버려진다**.
  // 900초 = 실측 최대(405s)의 2.2배. 상한은 지연이 아니라 뚜껑이라 빠를 땐 비용 0.
  PM:        900_000,
  TRADER:    360_000,
  DEBATE_R2: 300_000,
};
...
/**
 * 12-에이전트 파이프라인 전체 상한(ms).
 *
 * ★**`WORST_CASE_STAGE_MS` 보다 반드시 커야 한다.** 작으면 전체 abort 가 먼저 터져,
 * 단계별 실패 사유(timeout/cli-error)가 아니라 "중지"로 끝나 원인이 지워진다.
 * 직전까지 두 값이 정확히 3,000,000ms 로 같아 **슬랙이 0** 이었다 — 단계 상한을 올릴 때
 * 이 값도 함께 올릴 것. 불변식은 `__tests__/agentTimeouts.test.ts` 가 강제한다.
 */
export const TIMEOUT_TOTAL_MS = 3_600_000;
...
**편향 방지 원칙**: 강세·약세 주장에 동등한 가중치를 두세요. 발화 순서나 마지막 발언(구조상 약세가 토론·입력 모두에서 마지막에 옵니다)에 휩쓸리지 말고, 각 주장이 아래 [객관 검증 근거](기술 시그널·기술/뉴스/펀더멘털/심리 원자료)와 얼마나 부합하는지로 타당성을 판정하세요. 토론자가 인용한 수치·근거는 원자료와 대조해 검증하고, 과장·누락·왜곡이 보이면 그 주장의 가중치를 낮추세요.

양측 논거 중 데이터로 더 강하게 뒷받침되는 쪽이 있으면 명확한 입장을 취하세요. 양측 증거가 진정으로 균형 잡혀 있는 경우에만 보유(Hold)를 선택하세요.
...
**편향 방지**: 강세·약세 논거를 동등하게 검토하고, 제시 순서나 마지막 발언이 아니라 데이터 근거로 판단하세요.
${PCT_CLARITY}
...

{
  ...
  "stop_loss_pct": 판단 근거가 깨지는 라인(숫자, 현재가 대비 %). 강세(BUY/OVERWEIGHT/HOLD) = 하방 손절(음수, 예: -5). 약세(UNDERWEIGHT/REDUCE/SELL) = 상방 재검토 가격(양수, 예: +6 = 현재가 +6% 돌파 시 약세 논거 취소). 모든 verdict에 필수,
  "risk_reward_ratio": 손익비(숫자, 예: 3.0 = 3:1). BUY/OVERWEIGHT/HOLD에만 설정. UNDERWEIGHT/REDUCE/SELL = null,
  "short_term_outlook": "1~2주 단기 전망 (기술적 신호·수급·이벤트 중심 1~2문장). 구체적 방향·가격 레벨·트리거 제시, '박스권 등락' 같은 양비론 지양",
...
- **MEDIUM 남용 금지**: MEDIUM 을 기본값처럼 쓰지 마세요. 분석가·토론·리스크 팀이 대체로 같은 방향이고 데이터가 명확하면 주저 없이 HIGH, 신호가 실제로 상충하거나 데이터가 부족하면 LOW 로 명확히 가르세요. MEDIUM 은 방향은 잡혔으나 특정 근거끼리 진짜 이견이 있을 때로 한정합니다. (단, 아래 "데이터 제한 강제 규칙"이 우선 — 데이터 제한 마커가 있으면 HIGH 금지.)
...
**판단 품질 규칙 (reasoning 에 반드시 반영):**
- **반대 신호 명시 처리**: verdict 와 반대 방향을 가리키는 신호(약세 판정인데 RSI 과매도·외국인 연속 순매수·공매도 과다 누적·수주 촉매 대기, 강세 판정인데 기관 이탈·밸류에이션 부담 등)가 분석에 있으면, reasoning 에서 그 신호를 명시하고 **왜 최종 방향을 뒤집지 못하는지** 한 문장으로 반박하세요. 반대 신호를 key_strengths·key_risks 에 나열만 하고 reasoning 에서 침묵하는 것을 금지합니다.
- **시장 베타 vs 종목 알파 구분**: 예상 주가 흐름이 시장·섹터 전체 방향(베타)에 따른 것인지, 이 종목 고유 요인(알파)에 따른 것인지 구분하세요. 시황이 하락장이면 대다수 종목이 함께 빠지므로, verdict 는 **'시장 대비 이 종목이 더/덜 움직일 고유 이유'**에 근거해야 합니다. reasoning 에 이 종목이 동종업종·지수 대비 상대적으로 강한지/약한지를 한 문장으로 명시하세요. '시장이 빠질 것이므로 약세'처럼 방향을 시장에만 기대는 것은 알파 없는 판단입니다.
- **레벨은 제공된 실측값만 인용**: 입력의 [가격 레벨 — 실측값(추정 금지)] 블록에 이동평균·볼린저·피보나치·매물대의 **실제 가격**이 주어진다. 가격 레벨을 언급할 때는 그 값을 쓰고, 블록에 없는 지표(예: 일목균형표·특정 추세선)의 가격을 **추정해 단정하지 마라**. 특히 임의로 고른 가격에 "20일선"·"볼린저 하단" 같은 **기술적 라벨을 붙이는 것을 금지**한다(검증된 것처럼 읽혀 오해를 부른다). 근거가 필요한데 값이 없으면 라벨 없이 "심리적 지지" 수준으로만 서술하라.
- **재진입 구간은 실측 매물대 위에 찍어라**: 약세(UNDERWEIGHT/REDUCE)의 target_pct 는 "여기까지 빠지면 다시 볼 자리"다. 임의의 둥근 % 를 쓰지 말고, **[가격 레벨] 블록의 '현재가 아래(지지 후보)' 또는 '현재가 도달' 매물대 중 하나의 가격에 정확히 맞춰라**(그 구간 안이어야 한다). 매물대보다 **위**에 찍는 것도 틀렸다 — 그 아래 매물대까지 아직 하락 여지가 남아 있어 매수 타이밍이 아니기 때문이다. 매물대보다 **아래**에 찍는 것도 틀렸다 — 지지를 이미 깨고 내려간 자리라 받쳐줄 물량이 없다. 맞출 매물대가 없다(공백)면 target_pct 를 **null** 로 두고, 대신 (a) 현재 구간이 지지되는지 확인할 조건과 (b) 상승 추세 전환으로 볼 **돌파 레벨**을 전략에 명시하라 — 방향 베팅보다 확인 우선이면 verdict 도 HOLD 가 적절할 수 있다. ※ 서버가 이 앵커를 검증해 어긋나면 target_pct 를 자동으로 null 처리한다.
- **매물대 배치를 판단 근거로**: 현재가가 매물대를 이미 깨고 내려왔으면 아래 첫 매물대까지 추가 하락 여지가 있고, 매물대에 도달·근접했으면 지지 시도 구간이다. 위쪽 매물대는 반등 시 저항이자 돌파 시 추세 전환 트리거다. target_pct·stop_loss_pct 를 정할 때 이 배치를 우선 참고하라.
- **전망은 구체적·검증가능하게**: short_term_outlook 은 '박스권 등락·방향성 부재' 같은 양비론 대신 구체적 방향·가격 레벨·트리거 조건을 제시하세요(예: "X원 지지 실패 시 Y원 테스트", "Z 저항 돌파 시 모멘텀 전환"). 사후에 맞았는지 판별 가능한 문장이어야 합니다.

stop_loss_pct(판단 근거가 깨지는 라인) 설정 기준 — 방향이 verdict로 갈립니다:
- BUY/OVERWEIGHT/HOLD: 하방 손절(음수). 기술적 지지선 이탈 지점.
- UNDERWEIGHT/REDUCE/SELL: **상방 재검토 가격(양수)**. 약세 논거가 깨지는 저항 돌파 지점. 현재가가 이 위로 오르면 약세 판단을 철회해야 하는 라인이며, 하방 손절이 아님. target_pct(하방 재진입 구간)와 반대 방향이어야 함. ※ 서술형 텍스트에서 이 레벨을 언급할 때는 화면 표기와 맞춰 **"재검토 가격"** 이라 쓰고 "무효화" 라는 말은 쓰지 마라.

**폭은 고정 % 가 아니라 이 종목의 변동성에 맞춰라.** [가격 레벨] 블록의 **통상 변동폭**을 기준으로 잡되, 라인이 그 안에 들어가면 논거가 깨져서가 아니라 **노이즈로 발동**한다(그 라인의 의미가 사라진다). 판단 기간(time_horizon)에 대응하는 통상 변동폭보다 **밖**에 두는 것을 원칙으로 하고, 그보다 좁게 잡을 때는 왜 그 레벨이 노이즈가 아닌 구조적 경계인지(매물대·직전 저점/고점 등) reasoning 에 근거를 대라. 종목마다 통상 변동폭이 크게 다르므로(실측 2.4%~13.3%) 모든 종목에 같은 % 를 쓰는 것은 틀린 접근이다.
