...
- UNDERWEIGHT/REDUCE/SELL: **상방 무효화(양수)**. 약세 논거가 깨지는 저항 돌파 지점. 현재가가 이 위로 오르면 약세 판단을 철회해야 하는 라인이며, 하방 손절이 아님. target_pct(하방 재진입 구간)와 반대 방향이어야 함.

**폭은 고정 % 가 아니라 이 종목의 변동성에 맞춰라.** [가격 레벨] 블록의 **통상 변동폭**을 기준으로 잡되, 라인이 그 안에 들어가면 논거가 깨져서가 아니라 **노이즈로 발동**한다(무효화 라인의 의미가 사라진다). 판단 기간(time_horizon)에 대응하는 통상 변동폭보다 **밖**에 두는 것을 원칙으로 하고, 그보다 좁게 잡을 때는 왜 그 레벨이 노이즈가 아닌 구조적 경계인지(매물대·직전 저점/고점 등) reasoning 에 근거를 대라. 종목마다 통상 변동폭이 크게 다르므로(실측 2.4%~13.3%) 모든 종목에 같은 % 를 쓰는 것은 틀린 접근이다.
