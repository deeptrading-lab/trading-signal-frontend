...
- **재진입 구간은 실측 지지에 앵커**: 약세(UNDERWEIGHT/REDUCE)의 target_pct 는 "여기까지 빠지면 다시 볼 자리"다. **임의의 둥근 % 를 쓰지 말고 아래쪽 매물대·볼린저 하단 등 제공된 실측 지지 레벨에 맞춰라.** 아래에 받쳐줄 매물대가 없다(공백)면 그 구간은 지지가 아니라 **낙폭이 확대되는 위험 구간**이므로, 그런 자리를 재진입가로 제시하는 것은 모순이다. 이 경우 target_pct 를 null 로 두고, 대신 (a) 현재 구간이 지지되는지 확인할 조건과 (b) 상승 추세 전환으로 볼 **돌파 레벨**을 전략에 명시하라 — 방향 베팅보다 확인 우선이면 verdict 도 HOLD 가 적절할 수 있다.
