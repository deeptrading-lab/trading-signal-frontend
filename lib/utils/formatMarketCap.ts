/**
 * 시가총액 컴팩트 표기 — 실시간 순위 시총 컬럼(⑤ `ranking-columns`).
 *
 * 시총 원값(원)은 자릿수가 길어 콤마 표기(`formatMoney`)로는 셀이 넘친다. 토스식 조/억 컴팩트로
 * 좁은 순위 셀에 맞춘다(`formatNetBuy` 억원 패턴 답습·NaN 방어). 예:
 *   - `173_400_000_000_000` → `"173.4조"`
 *   - `845_000_000_000`     → `"8,450억"`
 *   - `null`·`0`·`NaN`      → `"-"`(fail-soft — 토스 미설정·enrich 실패)
 */
...
/** 시총(원) → "N조"/"N억"/"-" 컴팩트 표기. 미확보·비정상 값은 "-"(NaN 방어). */
