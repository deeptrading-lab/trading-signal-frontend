 ...
 * 라벨 타이포는 `text-badge`(DESIGN processing-badge typography). 활동 표시는 **펄스 점**으로
 * 라이브 진행 스피너와 명확히 구분한다(DESIGN R3). 점은 aria-hidden, 가시 텍스트가 곧 접근성
 * 이름이다. aria-live 미사용(15s 주기 갱신 ambient).
 */
...
const PILL_BASE =
  "inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-badge break-keep";
...
export function WorkerActivityBadge() {
  ...
  // processing | queued — 둘 다 "활동 중" 옅은 파랑. processing 은 펄스 점(실행 중), queued 는 정적 점.
  ...
  return (
    <span className={cn(PILL_BASE, "bg-accent-vivid-soft text-primary")}>
      <span className={cn(DOT, isProcessing && "animate-pulse")} aria-hidden />
      {label}
    </span>
  );
}
