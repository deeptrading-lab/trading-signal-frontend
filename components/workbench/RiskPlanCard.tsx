...
export function RiskPlanCard({ riskPlan, currency, isUnrealistic }: Props) {
  ...
  return (
    <article className="card riskCard" aria-label="리스크 플랜">
      <p className="resultBlockTitle">리스크 플랜</p>
      {isUnrealistic ? (
        <p className="riskUnrealisticNote">
      ...
      <div className="priceBarLegend">
        <span className="legend-stop">손절</span>
        <span className="legend-entry">진입</span>
        <span className="legend-target">익절</span>
      </div>
      ...
      <table className="riskTable">
        <tbody>
          <tr>
            <th scope="row">진입가</th>
            <td>{formatMoney(entry_price, currency)}</td>
          </tr>
          <tr>
            <th scope="row">손절가</th>
            <td>{formatMoney(stop_loss_price_for_day, currency)}</td>
          </tr>
          <tr>
            <th scope="row">익절가</th>
            <td>{formatMoney(take_profit_price_for_day, currency)}</td>
          </tr>
          <tr>
            <th scope="row">제안 매수 금액</th>
            <td>{formatMoney(suggested_buy_amount, currency)}</td>
          </tr>
          <tr>
            <th scope="row">제안 수량</th>
            <td>{formatNumber(suggested_share_qty, { digits: 4 })}</td>
          </tr>
          <tr>
            <th scope="row">손절 시 예상 손실</th>
            <td>{formatMoney(expected_loss_if_stopped, currency)}</td>
          </tr>
          <tr>
            <th scope="row">익절 시 예상 이익</th>
            <td>{formatMoney(expected_gain_if_take_profit, currency)}</td>
          </tr>
        </tbody>
      </table>
      <p className="rrRow">
        손익비 <strong>{formatNumber(risk_reward_ratio, { digits: 2 })} : 1</strong>
      </p>
    </article>
  );
}
