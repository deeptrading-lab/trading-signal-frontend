...
2. 선택 → `/stock/AAPL` → **현재가·등락·일봉 차트·호가·기본정보(시총 포함)가 표시**된다(공시/재무/수급 섹션은 숨김 — 뱃지·안내 없음).
...
- **R3** US 종목 상세: 현재가·등락률·일봉 차트·호가·기본정보·시총. KR 전용 섹션(공시/재무/수급)은 US에서 **완전 숨김**(뱃지·안내 박스 없음 — Q6 확정).
...
**인프라/운영**
- **prod Toss 활성화는 보류·비범위**(Q3 확정) — 로컬(`.env.local` TOSS 키)에서만 US 검증·개발. prod 반영 결정은 로컬 검증 후 별도. Toss는 KR 데이터 소스에도 영향([toss-api-evaluation]) → 신중.
- US 심볼 소스 = NASDAQ Trader HTTP 미러(`nasdaqtrader.com/dynamic/SymDir/`) 도달 확인(200). 무키·무료. CI(GHA)에서 IP 차단 가능성은 자동화 연동 시 재확인.
...
- **Q3. P1 데이터 소스/Toss 활성화** — prod에서 Toss 켤 것인가, US만 Toss로 라우팅할 것인가. → **✅ 확정(사용자)**
  - **로컬 검증 우선 + 티커 포맷 기반 라우팅**(US=Toss, KR=기존 KIS 유지). **prod Toss 키 투입·전역 `MARKET_DATA_SOURCE=toss` 전환은 보류** — 먼저 로컬(`.env.local` TOSS 키 존재)에서 US 데이터 확인.
  - **✅ 2026-07-10 로컬 스파이크 검증 완료**: `getTossStockMaster`/`fetchStockPriceToss`/`fetchStockDailyToss`/`fetchOrderbook` 로 AAPL·NVDA·TSLA 실측 — 한글명·거래소(NASDAQ)·상장주식수·USD·현재가·일봉 30봉·호가 전부 정상. **P1 표시는 로컬에서 기술 검증됨**(prod 결정 없이 착수 가능).
...
- **Q6. degrade UX** — US에서 공시/재무/수급 섹션을 숨길지 "미지원" 안내할지. → **✅ 확정(사용자)**
  - **KR 전용 섹션(공시·재무·수급)은 US 종목에서 완전 숨김. 별도 뱃지·"미지원" 안내 박스 없음** — 값 없는 섹션은 그냥 렌더 안 함(빈 박스 금지).
