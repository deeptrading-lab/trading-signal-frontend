...
- **q1. toss 과거 일봉 close ≠ KIS close 케이스**: NXT 반영 여부 실측 후, 불일치면 스코어카드 채점 경로만 KIS 고정(함수 분리) 검토. — *PM 권고: E2E 대조 결과를 QA 리포트에 수치로 남기고, 불일치 0이면 그대로, 발견 시 후속 PR.*
  → **[RESOLVED 2026-07-02, QA E2E 실측] 불일치 확인 — 토스 일봉은 KRX+NXT 통합 시세.** 확정봉 전수(29/29, 400일 267/267)에서 OHLC 소폭·거래량 +70% 수준 상이(버그 아닌 정의 차이, `docs/qa/toss-market-data-adapter.md` AC-4). 현 스코프 영향 0 — prod cron 채점은 토글 미설정(=kis)이라 채점 기준 불변. **토스를 prod/채점 경로까지 확장하는 시점에 스코어카드·백테스트 캔들 소스 일관성 처리(채점 KIS 고정 or 전면 전환 후 재기준) 후속 PR 필수.**
