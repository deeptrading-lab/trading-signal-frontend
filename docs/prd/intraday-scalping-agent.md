...
## 0. 검증 결과 & 방향 전환 (2026-06-28) — ⚠️ AUTO-EDGE **NO-GO** → decision-support 피벗

> **이 §0가 아래 §1~§9의 "자동 net 엣지" 전제를 무효화한다.** 데이터·파이프라인은 전부 구현·동작하나, **전략에 재현 가능한 엣지가 없음**이 검증으로 확정됐다.

**검증 (오프라인 진단 하네스: `lib/signal/backtest/__live__/intradayBacktest.test.ts` + `intradayDiagnostic.test.ts`)**
- **표본 1 (편향)** — 고수급 4종목(전부 하락) × 10일: gross 양수처럼 보였으나 롱/숏 분리 결과 **전량 숏 편향**(샘플 하락 방향).
- **표본 2 (균형)** — 섹터 분산 12종목 × 15일(종목당 ~4200 1분봉): **풀링 net@0.4% 전 TF 음수**(3m PF 0.48 / 5m 0.57 / 15m 0.87), **0/12 합격, 풀게이트 FAIL.**

**핵심 발견**
1. **방향 중립 net 엣지 없음.** 균형 샘플에서도 롱은 전 TF 음수(PF 0.84~0.87)·숏만 양수 — 숏 우위는 시장 베타(하락기)와 교락. **거래비용(왕복 0.4%, 국내 매도세만 0.18%)이 binding constraint.**
2. **분봉 추세추종 매수 신호가 구조적 역예측** — attribution: 골든크로스 17%·MACD상향 29%·거래량급증 27% 적중. 일봉(추세 지속)과 정반대로 분봉 크로스는 국지 고점 추격 → 손실.
3. 유일한 빛 15m gross PF 1.41(비용 0.2% 가정 시 +0.08%)도 0/12·노이즈 수준 → **거래 가능 엣지 아님.**

**결정 (D-pivot)**
- 결정론 룰을 더 튜닝하면 이 하락기 데이터에 대한 **곡선 과적합**일 뿐 → **튜닝 중단.**
- "자동 수익 스캘퍼" 전제 폐기. 이미 구현된 자산(분봉 데이터·결정론 레벨·2-에이전트 그룹·paper-trading·scorecard)을 **"사람을 위한 의사결정 보조(decision-support)"** 로 재포지셔닝한다.
- **실/가상 자동 수익 집행을 제품 가치로 주장하지 않는다.** 신호·근거를 사람이 읽고 **사람이 집행** (= 유저가 처음 던진 "AI 직접매매 vs 사람집행"의 데이터 기반 답).
- **검증 게이트가 음의 기대값 위에 LLM·자동화·실집행을 얹는 것을 막아준 것이 본 작업의 성과.** 향후 누가 재시도해도 같은 길을 안 가도록 본 §0를 박제.

decision-support 재정의 상세는 후속 작업(본 PRD 개정 또는 `intraday-decision-support` PRD)으로 잇는다.
...
### RESOLVED (2026-06-28) — §0 검증 결과로 무효화
- **Q1(타임프레임)·Q3(신호-only 표면)·Q5(LLM 권한)** 는 모두 "자동 net 엣지 존재"를 전제했으나 §0에서 **엣지 없음 확정**으로 무의미해졌다.
- **D2(가상매매 자동집행)** → **decision-support 로 전환**: 자동 집행/수익 주장 폐기, 사람 집행. Q3의 "신호-only 표면"이 오히려 **decision-support 의 핵심 산출물**이 된다(엣지 검증 후가 아니라 지금 바로).
- 검증 게이트(§3-3)·진단 하네스는 **회귀 자산으로 보존** — 후속 시그널 가설 검증에 재사용.
