...
- **q1. `FHPTJ04400000` 반환 행 수 = 누적 정확도** — 전시장인지 상위 N행인지 미검증. → **PM 권고: cron 1차 배포로 행 수를 로깅(PoC)** 후 근사 오차 판단. 충분히 넓으면 그대로, 좁으면 카피로 "상위 누적" 한정 표기. (구현 차단 아님 — 근사 전제로 진행.)
- **q2. 라우트 형태** — 기존 `top10`에 `mode` 파라미터 vs 신규 `top10-cumulative`. → **PM 권고: `mode` 파라미터**(중복 최소, 게이트/타임아웃 로직 공유).
- **q3. UI** — 당일↔누적 **토글**(한 카드) vs **별도 카드 2개**. → **PM 권고: 토글**(홈 정보밀도·세로길이 절약, 토스톤).
- **q4. cron 시각** — KST 16:10 적정? 가집계 확정 타이밍. → **PM 권고: 16:10(07:10 UTC)**, 운영 보며 조정(빈응답이면 더 늦게).
