...
- `/`(홈) 진입 시 **시장 종합 화면**이 렌더된다: 주요 지수(KOSPI/KOSDAQ/KOSPI200 실데이터) + 종목 검색 + 시장심리(§9 q1). 개별종목분석 mock(`HomeDashboard`) 은 제거된다.
- 계좌 위젯(총자산·투자원금·평가손익·손익률·자산비중·보유종목) 이 `/profile` "내 자산" 섹션에서 렌더된다. `/dashboard` 라우트 제거 + 진입 처리(§9 q4).
- nav 가 6 → 4(홈·관심종목·마이페이지 + 하단 AI분석)로 정리되고, **AI 분석은 사이드바 하단 + "준비 중" 시각 처리**로 노출된다. `/market` 진입 처리 정의(§9 q4).
...
- 자산비중(주식/코인) 은 현 바 → **도넛 차트** 재시각화 권장(디자이너 합류 시 확정). 보유종목은 Top3 → 전체 테이블 확장 여지(1차는 Top3 유지 가능).
...
- 1차 위젯 후보:
  ...
  - **시장 분위기 카드(간이 시장심리)** 🟡 — §9 q1=(c) 확정. **정식 "Fear & Greed" 아님** — 한국 증시 F&G 는 공개 API 가 없어(리서치 §1, 개인 대시보드는 스크래핑·약관 리스크, CNN 은 미국 전용 비공식 API, 코인 F&G 는 무관) 외부 연동 불가. 대신 **이미 호출 중인 지수 응답의 등락종목수(`ascn_issu_cnt`/`down_issu_cnt`)로 breadth 기반 0~100 점수를 자체 산출**(신규 연동 0). 상세 설계·해석 카피는 §3.5.
  - **최신 공시 피드** 🟡 — §9 q2=(a) **포함 확정**. `/api/disclosure/list`(OpenDART). 설계·쿼터 가드는 §3.6.
...
- 홈은 `useQueryIndices` 등 **커스텀훅만 소비**(frontend.md §1). 지수 카드 client 경계만 분리, 나머지 server 우선.
...
- **AI 분석(`/analyze`)**: nav 에서 제거하지 않고 **사이드바 하단 영역으로 이동** + **"준비 중" 버튼 디자인**(예: dimmed + "준비 중" 배지, 또는 비활성/툴팁). "엔진 레포 연동 후 정식화" 의도(§1.1.1). 페이지 자체(`/analyze`)·`components/workbench/*` 는 **무변경으로 살려둠**.
  - 구현 옵션: NAV_ITEMS 를 메인 그룹 + 하단(준비중) 그룹으로 분리하거나, NavItem 에 `status: "ready" | "comingSoon"` + `placement: "main" | "bottom"` 플래그 추가. Sidebar/BottomNav 렌더 로직 보강. 최종 형태는 디자이너 + dev.
...
### 3.5 간이 시장심리(breadth) 위젯 설계 — §9 q1=(c)
...
- **명칭 가드**: 위젯명은 **"오늘의 시장 분위기" / "시장 폭"** 류. **정식 "Fear & Greed"·CNN 브랜딩 사용 금지**(정본 혼동·과신 방지).
- **구간 라벨 + 초보자 해석 카피**(역발상 통념 + 리스크 고지 — 사용자 요구 "F&G 뜻 모르는 사람도 지금 매매 좋/나쁨 알게"):
  | 점수 | 라벨 | 한 줄 카피 |
  |---|---|---|
  | 0–24 | 극단적 공포 | "시장이 과도하게 공포에 빠져 있어요 — 역사적으로 분할매수 관점이 거론되는 구간. 단, 지표일 뿐 신중히." |
  | 25–44 | 공포 | "투자심리가 위축돼 있어요. 변동성 큰 구간 — 분할·분산 관점이 자주 언급돼요." |
  | 45–55 | 중립 | "한쪽으로 치우치지 않은 중립 구간이에요. 추세 확인이 필요해요." |
  | 56–75 | 탐욕 | "투자심리가 달아오르고 있어요. 추격매수는 신중히, 리스크 관리를 함께." |
  | 76–100 | 극단적 탐욕 | "과열·탐욕 구간이에요 — 역사적으로 조정 위험이 거론돼요. 단, 지표일 뿐 신중히." |
- **디스클레이머(상시 노출, 필수)**: "이 점수는 KIS 등락종목수 기반의 참고용 보조지표이며, 투자 판단의 단독 근거가 아닙니다."
- 카피는 `lib/copy/home/labels.ts`. 시각화는 반원 게이지(디자이너 확정).
...
- 시장분위기/공시: 스켈레톤 + 한글 에러 카피(`lib/copy/home/labels.ts` 신설).
...
- **정식 Fear & Greed 지수(외부 소스/7요소 자체산출)** — 한국 증시 F&G 공개 API 부재(리서치 §1·§2), VIX/Put-Call/채권 4요소 현 스택 불가. 1차는 §3.6 간이 breadth 점수로 대체. 정식화는 데이터 소스 확보 시 후속.
- **시장 분위기 2차 확장**(지수 모멘텀·거래량 가중, 일자별 N일선) — 1차는 breadth 단독.
...
### AC-2 계좌 위젯 마이페이지 이전
- `git grep -rn "PortfolioHero\|HoldingsTop3\|내 자산" components/profile` → 1건 이상.
...
### AC-4 라우트 제거 처리
- `/dashboard`·`/market` 진입 시 not-found 또는 리다이렉트(§9 q4 결정대로). `find app/\(main\)/dashboard app/\(main\)/market -name page.tsx` → 0건(제거 시).
- `/`·`/analyze`·`/watchlist`·`/profile` 정상 동작.

### AC-5 지수 실데이터 홈 노출
- 홈에서 KOSPI/KOSDAQ/KOSPI200 카드 렌더. DevTools Network `/api/market/indices` + `X-Data-Source` 헤더(prod 키 시 kis, 그외 mock) 확인.

### AC-6 간이 시장심리 위젯 (§3.5)
- 홈에 시장 분위기 카드 렌더. 위젯명/카피에 정식 "Fear & Greed"·"CNN" 문자열 **미사용**: `git grep -rn "Fear.*Greed\|FearGreed\|CNN" components/home lib/copy/home` → 0건.
- breadth 점수 산출 유틸 + 구간 라벨/디스클레이머 카피 존재: `git grep -rn "디스클레이머\|참고용 보조지표\|시장 분위기\|시장 폭" lib/copy/home` → 1건 이상.
...
- 지수는 prod 전용(모의 미지원) — 홈에서도 `KIS_ENV=prod`+키 설정 시만 실데이터, 그외 mock fallback(이중 게이트 유지).
- F&G·등락종목수 실데이터 소스 미확정 — 1차 mock 또는 보류(§9 q1).
...
| 영역 | 파일 | 추정 라인 | 성격 |
|---|---|---|---|
| 홈 셸 재작성 | `app/(main)/page.tsx` + `components/home/*`(시장종합 신규) | ~200 | 교체(기존 9컴포넌트 제거 + 신규) |
| 계좌 이전 | `components/profile/*`(자산 섹션) + `app/(main)/profile/page.tsx` | ~150 | 이동+재구성 |
| 지수 카드 이전 | 홈에서 IndicesCard 재사용 | ~40 | 재배치 |
| nav/사이드바 정리 | `navItems.ts` · `navCopy.ts` · `Sidebar.tsx` · `BottomNav.tsx`(AI분석 하단+준비중 상태) | ~80 | 수정(상태 플래그+렌더) |
| 라우트 제거/리다이렉트 | `app/(main)/{dashboard,market}/` | ~-60 | 삭제(+redirect) |
| mock/타입 이동 | `lib/mock/{dashboard→profile}` · `lib/types/*` | ~60 | 이동 |
| copy 신설 | `lib/copy/home/labels.ts` | ~30 | 신규 |
| **합계** | | **~560(순증 ~500)** | |

### 8.2 PR 분할 권고

- **2-PR 분할 권고**(최종 확정 §9 q5):
  - **PR1 — 계좌 이전**: 계좌 위젯 `/dashboard`→`/profile`, `/dashboard` 제거, mock/타입 이동. 홈 무관, 독립 머지 가능.
  - **PR2 — 홈 시장종합 + nav/사이드바 정리**: 홈 교체, `/market` 흡수+제거, AI분석 하단+준비중, 진입 동선.
- 사유: 회귀 영역 분리(라우팅/사이드바 vs 자산 정보). PR2 의 디자이너 왕복이 PR1 을 막지 않게.
- **대안(단일 PR)**: 재기획 한 흐름이라 single-PR 룰 엄격 적용 시 1 PR(커밋 분할 권고).

### 8.3 회귀 위험

- **중** — nav 라우트 제거 시 기존 `/dashboard`·`/market` 북마크/링크 깨짐. redirect(§9 q4=b) 또는 catch-all 로 완화.
...
- **저** — 지수 카드 재사용 시 `/market` 전용 가정(테마 그리드) 분리 필요.

### 8.4 후속 PR 자연 연결

- **수급 페이지(`investor-flow-page` 가칭) — 우선 후속(사용자 관심 큼).** 외국인/기관 투자자 매매동향. KIS `foreign-institution-total`(FHPTJ04400000, 레퍼런스에 응답스펙 없음 → 영업/GitHub examples 확인 선행) 또는 `investor-trade-by-stock-daily`(FHPTJ04160001, 실전전용, 7일누적은 종목별 일별 합산) 다단 파이프라인. 사용자 요구: **최근 7일 누적 외국인/기관 순매수 상위 10 + 개인 순매도(−) 필터**. 별도 PRD 로 (1) KIS 수급 API 스펙 확정 → (2) BFF/집계 → (3) 전용 화면 + 정성 디자인.
- **AI 분석 엔진 레포 연동** — `/analyze` 를 "준비 중" → 정식화. 엔진 레포 연결 후 별도 트랙.
- **Top Movers 위젯** — 등락률/거래량 순위 API 연동 후 홈/시장.
- **F&G·등락종목수 실데이터 / 지수 스파크라인** — 소스·시계열 확보 후.
- **보유종목 전체 테이블·총자산 추이 차트** — 마이페이지 자산 섹션 확장. 실계좌 연동 시 mock→실데이터.

---

## 9. OPEN QUESTION → 전부 RESOLVED (2026-05-30)

> q1~q6 전부 사용자 확정. 본문(§3.2/§3.3/§3.6/§3.7/§4/§8) 에 반영됨.

- **[RESOLVED] q1 — 홈 시장심리 1차 처리 → (c) 등락종목수 기반 간이 시장심리 자체 산출 + 정식 F&G 보류.**
  F&G 데이터 확보 리서치 결과(2026-05-30): 한국 증시 F&G 공개 API **없음**(개인 대시보드는 KIS 스크래핑 기반·약관 리스크, CNN 은 미국 전용+비공식 API, 코인 F&G 는 무관), VIX/Put-Call/채권 등 정식 7요소는 현 스택 불가 → 외부 연동·정식 자체산출 모두 탈락. **이미 호출 중인 지수 응답 `ascn_issu_cnt`/`down_issu_cnt` 로 breadth 0~100 자체 산출**(신규 연동 0). 위젯명은 "시장 분위기/시장 폭"(정식 F&G 라벨 금지), 구간별 초보자 해석 카피 + 역발상 통념 + 리스크 디스클레이머 동반(사용자 요구 "뜻 모르는 사람도 매매 좋/나쁨 알게"). 반영: §3.2, §3.5, §4, AC-6.
- **[RESOLVED] q2 — 최신 공시 피드 → (a) 1차 포함.** OpenDART `/api/disclosure/list`. 1차는 관심종목/대표풀 한정 + DART 쿼터 가드(전체 시장 스캔은 별도). 반영: §3.2, §3.6, §4, AC-7.
- **[RESOLVED] q3 — 홈 라벨 + 수급 메뉴 → (a) 라벨 "홈" 유지 + 수급은 페이지 구현 시 nav 노출(1차 미노출).** 빈 메뉴 노출 지양, 홈 카드 동선으로 진입. 반영: §3.3, §3.4.
- **[RESOLVED] q4 — `/dashboard`·`/market` 제거 방식 → (b) 리다이렉트.** `/market`→`/`, `/dashboard`→`/profile`(기존 북마크 보존, redirect 한 줄). 반영: §2, §3.3, AC-4.
