...
## 리뷰 반영 (2026-07-03, 적대 리뷰 — CONFIRMED 0건 / PLAUSIBLE 3건)

- **F-1 반영**: 심볼 규칙 강화 — 길이 캡 20 + 영문숫자 1자 이상 의무(`isValidWarningsSymbol`
  단일 위치, 로더·BFF 공유). `.`/`..` 만으로 된 입력의 URL 세그먼트 이탈 차단 — 실서버 400 확인
  + 회귀 테스트 추가.
- **F-2 반영**: 로더 캐시 상한 512 (초과 시 오래된 키 축출) — 유니크 심볼 난사로 인한 Map 무한
  성장 차단 + 회귀 테스트(513개 적재 → 최초 키 재조회 발생·최신 키 히트).
- **F-3 보류**: 장외 마운트 시 refetchInterval 미기동(장 시작을 넘겨도 폴링 없음) —
  `useQueryMinuteChart` 와 동일한 기존 패턴이라 일관성 유지, 공통 개선은 후속.
- **F-4 보류(관측성 메모)**: 로더 내부 실패 시에도 `X-Data-Source: toss`(실패 vs 무경보 헤더
  구분 불가) + catch-all 무로그 — tossEnrich 와 동일한 fail-soft 설계 유지, 후속 검토.
