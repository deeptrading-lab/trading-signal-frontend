...
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  ...
  - Vercel 상세 폴링 — `resolveServerOperator()` 가 램다 호스트명이라 모든 세션이 "남의 세션" 판정 → 상세마다 Supabase 왕복. admin 게이트 1인 사용이라 수용했고, 문제 시 해법은 클라 분기가 아니라 `refreshForeignSessionTicks` 에 TTL 추가.
