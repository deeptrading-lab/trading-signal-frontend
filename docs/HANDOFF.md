...
### 2026-06-26 — feat(ai-analysis): PM 결정 카드 컴팩트 리디자인 + 목표·손절 절대가격 표기 (#160)

- **slug**: `verdict-card-redesign` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/160
- **요약**: feat(ai-analysis): PM 결정 카드 컴팩트 리디자인 + 목표·손절 절대가격 표기
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 요약
  > PM 최종 결정 카드(`FinalVerdictCard`)를 세로로 길던 레이아웃에서 컴팩트하게 재디자인하고, 핵심 정보 가시성을 높였다. 추가로 목표·손절·재진입을 **절대가격(±%)** 으로 표기하기 위해 분석 시점 현재가를 저장한다. 슬라이드 스위치 정렬·너비, PM 프롬프트 문체도 보정.
  > 
  > ## 변경 내용
  > - **헤더**: 신호 강도/확신도를 우측 강조 박스로 분리, 중복 근거 설명문 제거(툴팁으로 이동), "최종 결정" 배지와 겹치던 간격 확보.
  > - **매매 가이드**: 섹션·서브 타이틀 강조(`text-sm`/`text-[13px]`), 여백 축소, 통계 박스(재진입·손절·손익비)를 `라벨 좌 · 값 우` 한 줄 + PC 3열/모바일 1열.
  > - **절대가격 표기**: `FinalDecision.base_price`(분석 시점 현재가, JSONB라 마이그레이션 불필요) 저장 → 카드에서 `72,000(-11%)` 로 표기. base_price 없는 legacy 행은 기존처럼 `%` 만 폴백.
  > - **강점/리스크**: 단기·중기 전망과 동일하게 PC 2열·모바일 세로(`divide-y sm:divide-x`), 긴 내용 자동 줄바꿈.
  > - **슬라이드 스위치**: Claude 라벨을 우측 끝→가운데 정렬, 데스크탑 최소 너비 추가.
  > - **PM 프롬프트**: 텍스트 필드 **개조식·명사 종결 문체** 규칙 추가(명령형 "~하라/~하세요" 금지, 명사형 종결).
  > 
  > ## 영향 / 주의
  > - `base_price`·문체 변경은 **새로 분석한 결과부터** 적용된다. 기존 저장 카드는 재분석 시 갱신.
  > - 데이터 모델: `FinalDecision.base_price?: number | null` (optional → 기존 직렬화/테스트 무영향). DB 스키마 변경 없음.
  > - `proxy.ts`(SSE 미들웨어 제외)·`next-env.d.ts` 사전 변경은 이번 PR과 무관해 제외.
  > 
  > ## 테스트
  > - `tsc --noEmit` 통과.
  > - TODO(리뷰어/QA): 라이브 패널·`/analyze` 상세 시트에서 재분석 후 절대가격 표기, 강점/리스크 2열, 슬라이드 스위치 정렬을 PC·모바일 두 뷰포트에서 확인.
  > 
  > ## 다음 작업
...
### 2026-06-28 — feat(ai-analysis): 리서치매니저 객관 근거 주입 + 편향 방지 (#173)

- **slug**: `rm-judge-grounding` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/173
- **요약**: feat(ai-analysis): 리서치매니저 객관 근거 주입 + 편향 방지
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 요약
  > 토론 판정 단계(리서치매니저)의 **순서/마지막-발언 편향을 완화**하고 **판정 근거를 객관화**한다(중립 시그널 + 원시 리포트). 트레이더에도 편향 방지 1줄.
  > 
  > ## 왜 (구조적 문제 2건)
  > 1. **약세-라스트 쏠림**: 토론은 `강세R1→약세R1→강세R2→약세R2`라 약세가 항상 마지막 발언이고, **RM·트레이더·PM 프롬프트가 모두 강세→약세 순**이라 약세가 컨텍스트 마지막 = LLM recency로 약세 쏠림 가능. (PM엔 편향방지 문구가 있으나 RM·트레이더엔 없었음.) A/B 4종목 전부 UNDERWEIGHT 정황과 결이 맞음.
  > 2. **RM이 수사를 판정**: RM은 `priceContext`+양측 논거만 받음 → 원자료(ground truth)·중립 시그널 없이 "누가 설득력 있나"를 평가. 토론자 왜곡/누락을 대조할 데이터가 없음.
  > 
  > ## 변경 (lib/prompts/stock/aiAnalysis.ts)
  > - **RM system** 편향 방지 원칙: 동등 가중 · 순서/마지막발언 무관 · [객관 검증 근거]와 대조해 데이터 타당성으로 판정 · 과장/누락/왜곡 시 가중치↓.
  > - **RM user**: 양측 논거 **뒤**에 `[객관 검증 근거]` = `signalSummary`(결정론 중립 점수) + 기술/뉴스/펀더멘털/심리 **원시 리포트 4종**. → RM이 마지막으로 읽는 게 약세 주장이 아니라 **중립 원자료**(구조적 recency 완화) + ground truth 확보.
  > - **트레이더 system** 편향 방지 1줄.
  > 
  > ## 비용/무회귀
  > - RM=sonnet, 리포트는 기재생성 텍스트 재투입이라 캐시생성 ~$0.01/run(풀로 넣어 요약표 보존). 무회귀 불변식(config undefined==default) 유지 — analysisConfig.test 11 통과. tsc/eslint/vitest 32 통과.
  > 
  > ## 다음 작업
...
### 2026-07-02 — feat(intraday): 단타워치 ↔ AI 모의투자 통합 — 세션·자동틱·비용/분할·영속화·표 UI (#203)

- **slug**: `intraday-paper-watch` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/203
- **요약**: feat(intraday): 단타워치 ↔ AI 모의투자 통합 — 세션·자동틱·비용/분할·영속화·표 UI
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 요약
  > 
  > 자동매매 전 단계로 **AI 단타 판단 품질을 가상 수익률로 테스트**하는 통합. 단타워치(`/intraday`)에서 종목을 고르고 모의 투자금·판단 주기를 정하면, 장중 화면이 열려 있는 동안 주기마다 AI(2-에이전트 + 룰 게이트)가 판단하고 가상 체결한다. 판단·체결·비용·실현손익·판단 메모가 전부 Supabase 에 영구 축적돼 사후 분석의 원장이 된다.
  > 
  > 모의투자 엔진(세션/틱/가상체결)은 기존 ai-paper-trading 기능을 재사용하고, `runTick` 의 `cli-agent` seam 에 장중 단타 판단을 연결했다.
  > 
  > ## 주요 변경
  > 
  > **서버**
  > - 세션 생성 API `cli-agent` 개방(+같은 종목 running 세션 멱등 가드), 검증 `validateCreateSession` 모듈 추출
  > - 판단 주기 세션별 선택(1·2·3·5·10·15분) — 분봉 단위는 주기에서 자동 파생(`INTRADAY_TIMEFRAME_BY_INTERVAL`, 주기마다 최소 1봉 마감), env 는 실험용 오버라이드로 강등
  > - 거래 비용 모델(수수료 1.5bp/편도·매도 제세금 15bp·슬리피지 5bp/편도) — cli-agent 만 적용, mock 무회귀. 매도 주문에 실현손익 기록
  > - AI 분할 매수·매도: 판단가 JSON `entryPositionPct`/`sellRatioPct` → BUY=목표 비중(캡·역전 방지), SELL<100%=REDUCE, **HOLD=무주문**("빈 targetAllocations=리밸런싱 금지" 계약)
  > - 캔들 미시구조 피처(`lib/signal/intradayFeatures`): 마감봉 꼬리·스윙 구조(저점 붕괴/전고 돌파)·피보나치 되돌림·단기 박스·당일 컨텍스트(갭·전일 레벨)·VWAP·오프닝 레인지·RSI 다이버전스 → 프롬프트 주입. 판단 원칙(개미털기 vs 진짜 매도세, 물타기 금지 조건, 돌파 vs 눌림목, 주기 인지 horizon) 확장
  > - Supabase 영구 저장(`paper_trading_sessions/ticks`, payload jsonb): write-through + 부팅 hydrate(4s 타임아웃·오류 시 재시도·페이지네이션), `docs/sql/paper-trading.sql`
  > - 세션 단위 틱 직렬화(동시 호출 레이스 차단), 벽시계 창 고정(드리프트 제거)
  > - 거래량 순위 체인: KIS `volume-rank`(실전 전용) → `/api/market/volume-rank`(이중 게이트·mock 폴백·ETF/ETN 필터)
  > 
  > **UI**
  > - 단타워치: 최상단 공용 종목 검색(`components/ui/StockSearchPicker` 신설, 모의투자 생성 폼과 공용) → 추천 후보(수급·거래량) → **워치 표**(토스식: 현재가·등락률 | 모의 수익률·평가·포지션·최근 판단 | 투자금 콤보 input(프리셋)·주기 드랍다운·AI 진단/모의 매매/관리 컬럼)
  > - 행 펼침 = 진단 결과 카드 + 최근 체결 미니 로그, 🕘 = 체결 내역 시트(포털), ↗ = 세션 상세
  > - 세션 상세(전체 화면) 재디자인: 체결 내역 표(비용·실현손익·판단 메모)·실현손익 합/비용 누계 지표·자산 곡선 리디자인(차트 테마 SSOT·원화 축약·시각 X축·시작금 기준선)·타임라인 최신순(진단·룰 조정), 뒤로가기 `/intraday`
  > - 장중 자동 틱 훅(60초 체크·서버 창 dedup·마감 유예 15:40) — 워치에 올린 종목만
  > 
  > ## 검증
  > 
  > - tsc·eslint 클린, vitest 711 통과(비용 검산·분할 매핑·피처 추출·윈도/직렬화/멱등 등 신규 다수)
  > - 멀티에이전트 코드리뷰(high): 32후보 → 확정 10건 전부 수정 완료(HOLD 매도 누수·동시 틱 레이스·hydrate 절단·hydration 블로킹 등)
  > - 실데이터 검증: 7/2 삼성전자 실분봉 3컷에서 피처 불변식 전부 통과·장세 정합, Supabase 저장·복원 라이브 실증
  > - ⚠️ 장중 라이브 E2E(실 LLM 판단·분할 체결·15:20 청산)는 금일 장에서 확인 예정
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - 장중 라이브 E2E 확인(2분 주기 실판단 → 체결·손익·메모 축적 → 15:20 자동 청산) 후 머지
...
### 2026-07-05 — feat(home): 실시간 순위 값 모바일 노출 (종목명 아래 캡션) (#263)

- **slug**: `mobile-ranking-value` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/263
- **요약**: feat(home): 실시간 순위 값 모바일 노출 (종목명 아래 캡션)
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경
  > 실시간 순위 값 컬럼(#256/#261)은 **md+ only** 라, 모바일에선 활성 탭이 무엇으로 줄 세운 순위인지(거래량/거래대금)가 안 보였다. 현재가·등락률만으론 정렬 기준을 알 수 없다. 토스도 모바일에서 이 값을 노출한다.
  > 
  > ## 변경 (경량 UX 폴리시)
  > 종목 셀(모바일 flex-col)에 **모바일 전용 muted 캡션**을 추가 — 종목명 아래에 "거래대금 5.3조" / "거래량 1,823만주" 형태로 활성 탭의 값을 표시. `md:hidden` 이라 md+ 에선 기존 전용 컬럼이 담당(중복 없음).
  > 
  > - 기존 `valueColumn`(4탭 통일) 그대로 재사용 → 거래량 탭=거래량, 나머지=거래대금.
  > - **신규 클래스 리터럴·토큰 0** (text-caption·tabular-nums·text-text-muted·md:hidden 전부 기존 사용).
  > - fail-soft: 미보유 값은 포맷터가 "-".
  > 
  > ## 무회귀
  > - md+ 레이아웃·전용 값 컬럼·경고배지·위험숨기기·가용성 전부 무변경.
  > - 모바일 행이 값 캡션 한 줄 만큼 늘어남(정보 밀도 ↑, 토스 패리티).
  > 
  > ## 테스트
  > - `tsc --noEmit` clean · `eslint` clean. 새 클래스 없음 → 빌드 저위험(QA 빌드 검증).
  > - 격리 worktree 작업(공유 트리 병렬 세션 충돌 회피).
  > 
  > ## 다음 작업
  > - 값 컬럼 정렬 옵션·기간별 랭킹(1일~1년).
...
### 2026-07-05 — feat(peek): 우측 도크 가변 폭 (여백에 맞춰 차트 크게) (#268)

- **slug**: `peek-dock-variable-width` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/268
- **요약**: feat(peek): 우측 도크 가변 폭 (여백에 맞춰 차트 크게)
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경
  > #260 우측 도크는 폭이 **고정 248px**이라, 2560px 같은 초광폭에서도 1920px과 똑같이 작았다. 사용자가 원한 "좀 더 크게"를 화면 여유에 맞춰 살린다.
  > 
  > ## 변경 (경량 UX 폴리시)
  > 도크 폭을 **콘텐츠(1152px) 우측 빈 여백 실측**에 맞춰 스케일:
  > - 여백 = `(main 실폭 − 1152) / 2`, 도크 폭 = `여백 − 우측갭(16) − 최소간격(12)`, `[248, 400]` 클램프.
  > - 차트 높이도 폭에 비례(`220~360`).
  > - 1920px(사이드바 확장): 여백≈280 → 폭≈252(≈기존). 2560px: 여백≈600 → 폭 400(상한, 훨씬 큰 차트).
  > - `useLayoutEffect` + resize 리스너로 실측·갱신(페인트 전, 깜빡임 없음).
  > 
  > ## 무회귀·안전
  > - `<main>` 미발견·여백 부족 시 **최소 248**(호스트의 `≥1920px` 게이트가 최소 여백을 이미 보장 → 겹침 없음).
  > - `MAIN_MAX_W`(1152)는 `spacing.main-max-w` 토큰 동기 레이아웃 상수(여백 산출용, 스타일 적용 아님).
  > - 폭은 runtime-positioned 인라인 style(팝오버 `PEEK_WIDTH` 관례) → **신규 Tailwind 클래스·토큰 0**.
  > - 팝오버·시트·<1920px·모바일 무영향. pointer-events-none 등 #260 시맨틱 유지.
  > 
  > ## 테스트
...
### 2026-07-05 — fix(home): 지금 뜨는 산업 — 등락률·상승종목수 가로 컬럼 분리 (#279)

- **slug**: `sectors-columns` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/279
- **요약**: fix(home): 지금 뜨는 산업 — 등락률·상승종목수 가로 컬럼 분리
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경
  > 사용자 지적(스크린샷): "지금 뜨는 산업"이 아직 좀 큼. 원인 = 우측에 **등락률%와 "N개 중 M개 상승"을 세로 2줄로 스택** → 행 높이가 커짐.
  > 
  > ## 변경
  > 토스 증권 UI처럼 **별도 가로 컬럼**으로 분리 → 한 줄 유지, 행 높이 축소:
  > - 등락률 → 우측 고정폭 컬럼(`w-16 text-right`)
  > - 상승 종목수(breadth) → 등락률 왼쪽 별도 컬럼(`w-32 text-right text-caption`), **sm+ 노출**(좁은 모바일은 폭 부족으로 숨김 — 실시간 순위표의 부가 컬럼 md+ 관례와 동일 취지)
  > - `total===0` 업종은 breadth 빈칸(정렬 유지)
  > 
  > ## 무회귀·안전
  > - 클릭→구성종목 모달·가용성·색·순번 무변경. 신규 커스텀 Tailwind/토큰 0(w-16·w-32·hidden sm:block 등 기존 유틸).
  > 
  > ## 테스트
  > - `tsc` clean · `eslint` clean. 단일 파일·표현 변경.
  > 
  > ## 다음 작업
  > - (옵션) 모바일에서도 breadth를 보이려면 별도 처리 검토.
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - (옵션) 모바일에서도 breadth를 보이려면 별도 처리 검토.

### 2026-07-06 — perf(market): 구성종목 스파크라인 배치 레이트리밋 보강 (#282)

- **slug**: `sparkline-ratelimit` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/282
- **요약**: perf(market): 구성종목 스파크라인 배치 레이트리밋 보강
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경
  > 사용자 지적: 시가총액 탭(KIS 시총)을 붙이면 rate limit 안 걸리나? → 확인 결과 **앱에 전역 KIS 리미터가 없고**(기능별 동시성 캡만) 홈 위젯 겹침 시 이미 간헐 EGW00201(초당 한도 ~20/s)이 발생. #277 스파크라인 배치(30콜)도 **동시성 8·재시도 없이** 발사라 prod 라이브에서 일부 드롭 위험.
  > 
  > ## 결정
  > - **시가총액 탭(KIS 시총 계산)은 보류** — 토스는 IP 화이트리스트라 Vercel 불가, KIS 시총은 배치 엔드포인트 없어 구성종목당 +30콜(상장주수) → rate limit 리스크 가중. 부가 정렬이라 안 함(지금처럼 자동 숨김 유지).
  > - **대신 기존 스파크라인 배치를 rate-limit 안전하게 보강**(우선순위 높음).
  > 
  > ## 변경 (`loadSparklines` 3중 방어)
  > - **동시성 8→5** 축소.
  > - **배치 간 120ms 지연**(마지막 제외) — 초당 한도 버스트 억제.
  > - 각 콜을 **`fetchWithTransientRetry`** 로 감싸 EGW00201/네트워크 시 backoff 후 **1회 재시도**, 그래도 실패면 `[]`(생략) — fail-soft 유지(throw 없음).
  > 
  > ## 무회귀·안전
  > - 반환 계약 무변경(티커→종가 시리즈 맵, 미확보 생략). 클라/모달 무변경.
  > - 서버 로직만(UI·Tailwind 0). 기존 bffUtils 헬퍼(delay·fetchWithTransientRetry) 재사용.
  > 
  > ## 테스트
  > - `tsc` clean · `eslint` clean. 30콜 배치가 5동시성+지연+재시도로 초당 한도 여유.
  > 
  > ## 다음 작업
  > - prod 배포 후 구성종목 모달 열어 스파크라인 누락(EGW00201 드롭) 없는지 라이브 관찰.
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - prod 배포 후 구성종목 모달 열어 스파크라인 누락(EGW00201 드롭) 없는지 라이브 관찰.

### 2026-07-06 — fix(intraday): 호가창·체결강도 우측 sticky 도킹 (#284)

- **slug**: `intraday-orderbook-dock` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/284
- **요약**: fix(intraday): 호가창·체결강도 우측 sticky 도킹
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경
  > 사용자 지적: 단타 워치 페이지에 최근 추가된 호가창·체결강도(①#243·③#245)가 **워치 표 한참 아래에 세로로 쌓여** 떠서 어색하고 세로로 길다. 표 보다가 아래로 스크롤해야 함.
  > 
  > ## 변경 (사용자 선택: 우측 도킹 사이드)
  > 워치 표 + 선택 종목 상세를 **마스터-디테일 2컬럼**으로:
  > - **데스크탑(lg+)**: 좌 = 워치 표(flex-1), 우 = 호가창 + 체결강도 **sticky 도킹**(`lg:sticky lg:top-lg lg:w-[22rem]`) → 행 클릭 시 옆에 바로 뜨고 스크롤해도 따라옴.
  > - **모바일(<lg)**: 기존대로 표 아래 세로 스택(무회귀).
  > - 표는 이미 `overflow-x-auto` 래핑이라 좌측 컬럼이 좁아져도 가로 스크롤로 흡수(깨짐 없음).
  > 
  > ## 무회귀·안전
  > - 선택 상태·폴링·행 클릭 종목 전환 로직 무변경. compact 패널 그대로. 신규 커스텀 Tailwind/토큰 0(`lg:w-[22rem]`은 arbitrary 리터럴, 기존 `lg:max-w-[22rem]`과 동일 관례).
  > 
  > ## 테스트
  > - `tsc` clean · `eslint` clean. 단일 파일 레이아웃 변경.
  > 
  > ## 다음 작업
...
### 2026-07-09 — feat(intraday): 체결 마커 시인성 개선 — 원형 핀 + 흰 링/글자 (#328)

- **slug**: `feat/intraday-marker-pin` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/328
- **요약**: feat(intraday): 체결 마커 시인성 개선 — 원형 핀 + 흰 링/글자
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 요약
  > 
  > 직전 PR(#327)의 체결 마커(얇은 삼각형+같은 색 글자)가 **같은 색 캔들 사이에서 묻히는** 문제를 해결. 렌더 시안을 사용자와 반복 확정 후 **원형 핀(꽉 찬 색 원 배지 + 흰 링 2px + 흰 B/S 글자)** 으로 재구현.
  > 
  > ## 변경
  > 
  > - `components/intraday/IntradayMiniChart.tsx`:
  >   - 삼각형+글자 → **꼬리 삼각형(H10, 체결가 지시) + 색 원 배지(R9) + 흰 링 2px + 흰 글자**
  >   - 매수 B(빨강 theme.C.stroke) / 매도 S(파랑 theme.C.down) — 원색은 기존 캔들 테마 재사용
  >   - **흰 링·흰 글자**로 채도 높은 원 위 대비 확보(다크/라이트 공통), 캔들과 완전 분리
  >   - `tradeMarkerGeometry` 를 원형 핀 기하(tailPoints·circleX/Y·radius)로 갱신
  > 
  > ## 시안 확정 과정 (사용자 검수)
  > 
  > 렌더 미리보기로 반복 확정: 마커 스타일 A(원형 핀) → 꼬리 크기 A1 → 배지 테두리 흰색 링 2px.
  > 
  > ## 검증
  > 
  > - 기하 테스트 3건 갱신(꼬리 방향·원 위치·상하 대칭), 전체 vitest **1214 passed** / tsc·eslint 클린
  > - ⚠️ 흰색(#ffffff)은 **채도 높은 배지 위 고정 대비색**(테마 토큰 아님) — 링·글자에만 사용, 다크/라이트 무관하게 유효
  > 
  > ## 다음 작업
  > 
  > - 겹치는 체결(같은 분봉 다중 체결) 마커 오프셋 후속 여지
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - 겹치는 체결(같은 분봉 다중 체결) 마커 오프셋 후속 여지

### 2026-07-09 — feat(intraday): 단타 손절 신뢰성 A+B+C — 60초 리스크 스윕·하드스톱·세션별 손절 상한 (#333)

- **slug**: `intraday-stop-slippage` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/333
- **요약**: feat(intraday): 단타 손절 신뢰성 A+B+C — 60초 리스크 스윕·하드스톱·세션별 손절 상한
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경
  > 
  > 7/9 첫 실전 체결에서 **다스코**: 진입 5,473 · 손절선 **5,420(−1%)** · 실제 청산 **5,077(−7.96%)** —
  > 12:00봉 장대음봉이 손절선을 건너뛰고 급락했지만 판정이 **틱 주기(5분)에 묶여** 다음 틱(12:05)에서야 청산됐다. **손절선의 8배 손실.**
  > "버텼으면 됐다"가 아니다 — 데이터 검증: 오늘 손절 6건 중 청산 후 진입가 **회복 2건, 미회복 4건**(홀드는 표본상 지는 전략).
  > 진짜 개선은 **손절을 제대로(일찍) 작동**시키는 것. (PRD: `docs/prd/intraday-stop-slippage.md`)
  > 
  > ## 변경 (A + B + C)
  > 
  > ### A. 60초 리스크-only 스윕 (핵심)
  > - `tickScheduler` 매 60초 사이클에서 **LLM 5분 틱 앞**에 `runIntradayRiskSweep(now)` 실행 — owner-게이트로
  >   running cli-agent 세션의 **보유 포지션만 신선 가격으로** 검사(동적 손절선·익절가·하드스톱·장막판). **LLM 호출 0.**
  > - `sessionStore.runPaperTradingSessionRiskCheck`: LLM 틱과 **같은 tickChain 직렬화**로 조율(리스크 먼저 → LLM 틱은
  >   무포지션으로 봄, 중복 EXIT 없음). 무발동/무포지션/무신선은 **no-op**(60초 HOLD 틱 스팸 방지, 멱등).
  > - 최근 틱 `decision.targetPrice`/`invalidationPrice`(영속)에서 동적 손절선/익절가를 읽는다. 슬리피지 상한 5분 → ~60초.
  > 
  > ### B. 하드스톱 cli-agent 배선
  > - `resolveForcedExit` 에 포지션(`unrealizedPnlPct ≤ −N%`)·세션(`세션 수익률 ≤ −M%` flatten) 하드스톱 절 추가.
  > - `runTick` 이 세션 설정 하드스톱 + 신선 마크 세션 수익률을 forced-exit 에 주입 —
  >   **LLM 5분 틱과 A 60초 스윕이 같은 백스톱 공유(단일 지점)**. mock 경로 무영향.
  > 
  > ### C. 하드스톱 % 세션별 설정 (UI)
  > - `PaperTradingSession`/CreateRequest 에 `positionHardStopPct?`/`sessionHardStopPct?` 추가(payload jsonb, 무마이그레이션).
  > - 세션 생성 폼에 **'손절 상한' 셀렉트**(−3 보수 / −5 표준 / −8 공격 / **끄기**). '끄기'는 경고(물타기 무제한 리스크).
  > - 기본값 = riskMode 연동(보수 −3 / 균형 −5 / 공격 −8) + 명시 override. **'끄기'여도 동적 손절선은 유지**(하드스톱만 off).
  > 
  > ### 관측성 (AC-7)
  > - 손절/포지션 하드스톱 청산 시 `guardAdjustments` 에 **"손절선 5,420원 대비 실체결 5,077원 (−6.3%)"** 갭 기록 → 슬리피지 추적.
  > 
  > ## 검증
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
...
### 2026-07-10 — chore(data): 종목 검색 시드 최신화 — 신규 상장 3종목 추가·폐지 14종목 정리 (v0.4.0) (#342)

- **slug**: `chore/symbols-seed-refresh-0710` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/342
- **요약**: chore(data): 종목 검색 시드 최신화 — 신규 상장 3종목 추가·폐지 14종목 정리 (v0.4.0)
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경
  > 
  > 종목 검색(종목명 자동완성)은 라이브 API가 아니라 **저장소 번들 정적 시드** `lib/api/kis/symbols.json` 에 대한 클라이언트 매칭이다(`useQueryStockSearch` → `lib/api/kis/search.ts`). 이 시드는 수동 스크립트로만 갱신되고(npm/cron/CI 없음), **마지막 재생성이 2026-06-03(#110, v0.3.1)** 이라 이후 상장 종목이 검색·`getSymbolName`/`getCorpCode` 전부에서 누락됐다.
  > 
  > ## 변경
  > 
  > `python3 scripts/update-symbols.py` 재실행 — KRX KIND 상장법인목록 + OpenDART CORPCODE.xml 재수집. **2,611 → 2,600 종목**(KOSPI 830 + KOSDAQ 1,770), `$meta` v0.4.0 / createdAt 2026-07-10.
  > 
  > **신규 상장 3종목(직전 시드 이후, 이제 검색 가능):**
  > | 종목 | 티커 | 상장 | 시장 |
  > |---|---|---|---|
  > | 스트라드비젼 | 475040 | 2026-06-30 | KOSDAQ |
  > | 져스텍 | 153890 | 2026-06-29 | KOSDAQ |
  > | 레몬헬스케어 | 365660 | 2026-07-06 | KOSDAQ |
  > 
  > **상장폐지/스팩합병 14종목 제거**(검색 시 stale 결과 나오던 것): KB제27호스팩·NPX·노블엠앤비·바이온·비유테크놀러지·아이엠·아크솔루션스·에코마케팅·엔에이치스팩29호·프로브잇·한국제13호스팩·JK리버스톤리츠·에이리츠·일정실업.
  > 
  > **스크립트 보정**: `version` 하드코딩값 0.3.0 → 0.4.0(커밋본 0.3.1보다 역행하던 것 교정, 단조 증가) + note 에 KIND 6자리 숫자 티커 한계 명시.
  > 
  > ## 검증
  > 
  > - `tsc --noEmit` 0(JSON import 형태 정합).
  > - **실 검색 함수 exercise**(tsx): `searchSymbols("스트라드비젼"|"져스텍"|"레몬헬스케어")` 각 1건 정확 반환, `getSymbolName(475040|153890|365660)` 정확. 제거 종목 `searchSymbols("에코마케팅"|"일정실업"|"KB제27호스팩")` 전부 0건.
  > - 신규 3종목 상장일은 뉴스핌·이데일리 등으로 교차확인(전부 6/29~7/6 코스닥 신규상장).
  > 
  > ## 알려진 한계 / 다음 작업
  > 
  > - **KIND 상장법인목록은 6자리 *숫자* 티커만 수록** → 영숫자 코드(신 코드체계) 또는 당일 상장분은 며칠 지연 반영. 예: **매드업(7/1 코스닥)·한국제16호스팩(6/30)** 은 이번 재수집에도 아직 미포함. 며칠 뒤 재실행 시 편입 예상.
  > - **근본 과제**: 이 갱신이 100% 수동(npm/cron/CI 없음). 신규 상장 반영이 사람 손에 의존 → 주기적 자동 갱신(예: 주간 GitHub Actions로 스크립트 실행 후 PR) 별도 티켓 검토 여지.
  > 
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
...
### 2026-07-13 — feat(ai-analysis): PM 판정 캘리브레이션·추론 품질 프롬프트 강화 (#352)

- **slug**: `pm-prompt-calibration` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/352
- **요약**: feat(ai-analysis): PM 판정 캘리브레이션·추론 품질 프롬프트 강화
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경
  > 
  > AI 종합분석 사후검증 감사(#347·#350 후속)에서 나온 **판정 품질 결함**을 프롬프트로 교정합니다. 39종목 감사에서:
  > 
  > - **confidence 전원 MEDIUM** (39/39) — 확신도 캘리브레이션 기능이 사실상 무의미
  > - **자기 나열 반대신호 기각** — RSI 과매도·외국인 순매수·수주 촉매를 key_strengths/key_risks에 적어놓고 verdict는 반대로 (예: 주성엔지니어링 REDUCE 후 +46%, S-Oil UNDERWEIGHT 후 +54%)
  > - **베타/알파 미구분** — 절대 방향 적중 61% vs **초과수익(excess) 44%**. 하락장에서 시장 베타에 편승했을 뿐 종목 선별력(알파)은 동전던지기 이하
  > - **박스권 헤지 남발** — 단기전망 다수가 "박스권 등락"으로 수렴, 검증 불가
  > 
  > ## 변경 (프롬프트 4개, `lib/prompts/stock/aiAnalysis.ts` PM 시스템 프롬프트)
  > 
  > 1. **confidence MEDIUM 남용 금지** — 방향 합의·데이터 명확 시 HIGH, 상충·부족 시 LOW로 명확히 가르도록. MEDIUM은 진짜 이견이 있을 때로 한정
  > 2. **반대 신호 명시 처리** — verdict와 반대 방향 신호가 있으면 reasoning에서 명시하고 왜 방향을 못 뒤집는지 반박하도록 강제(나열만 하고 침묵 금지)
  > 3. **시장 베타 vs 종목 알파 구분** — '시장이 빠지니 약세'식 베타 편승 대신 동종업종·지수 대비 상대 강약을 명시하도록 요구(프롬프트에 이미 주입되는 시황 컨텍스트 활용). excess 적중률 개선 겨냥
  > 4. **전망 구체화** — short_term_outlook의 '박스권 등락' 양비론 억제, 검증가능한 방향·레벨·트리거 제시
  > 
  > ## 의도적으로 하지 않은 것
  > 
  > **verdict 방향 분포(약세 34/39·BUY 0)는 건드리지 않았습니다.** 감사 데이터가 KOSPI 3주 -20% 하락장이라 약세 다수가 오히려 정답일 수 있고, 진짜 결함은 방향이 아니라 알파 부재(변경 3번)입니다. 방향 분포를 억지로 맞추는 건 감사에서 경계한 "컷 튜닝=노이즈 추가" 함정이라 회피했습니다. (사용자 결정)
  > 
  > ## 검증 — 세션 내 불가, 스코어카드로 시간 두고 측정
  > 
  > 프롬프트 캘리브레이션 변경이라 판정 품질 효과는 **이번 세션에서 검증 불가**합니다. 효과는 이제 복구된 `signal_scorecard`(#347)가 신규 분석을 채점하며 누적돼야 측정됩니다.
  > 
  > 세션 내 확인한 것:
  > - JSON 스키마 **구조 무변경**(파싱 무영향) — 추가 텍스트 가이드만
  > - 타입체크·ESLint·관련 테스트 통과
  > - 조립된 PM 프롬프트에 4개 가이드가 올바른 위치(verdict 기준 ↔ stop 기준 사이)에 삽입됨 확인
  > 
  > ## 다음 작업
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
...
### 2026-07-13 — fix(ai-panel): 모바일 뒤로가기가 패널만 닫도록 히스토리 연동 (#361)

- **slug**: `ai-panel-back-close` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/361
- **요약**: fix(ai-panel): 모바일 뒤로가기가 패널만 닫도록 히스토리 연동
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경 (사용자 리포트)
  > 
  > 모바일에서 /analyze 카드를 눌러 AI 분석 패널(풀스크린 오버레이)을 연 뒤 **하드웨어 뒤로가기/스와이프**를 하면, 패널만 닫히길 기대했지만 라우트가 뒤로 가서 이전 페이지(예: 종목분석)로 이탈했다. 패널이 브라우저 히스토리에 얹혀 있지 않았기 때문.
  > 
  > ## 변경 (`hooks/stock/aiAnalysisProvider.tsx` +45줄)
  > 
  > - **열림**: 패널이 열리면(모바일 한정, `useBreakpoint`) marker 엔트리 1개를 `history.pushState` — 이후 뒤로가기(popstate)는 **패널만 닫고 라우트는 유지**
  > - **UI 닫기(X 등)**: `history.back()` 으로 marker 를 소비해 히스토리 잔여 없음(다음 뒤로가기가 정상적으로 이전 페이지로)
  > - **라우트 이동 collapse**: 현재 엔트리가 marker 가 아니므로 back() 없이 disarm 만(페이지 이탈 방지)
  > - 무장은 `isOpen` 전이 effect + armed ref — 카드 클릭·분석 시작·재열기 등 **모든 열림 경로** 커버, StrictMode 이중 실행 안전. push 지점이 한 곳이라 marker 중복 없음
  > - 데스크탑(사이드 패널)은 기존 동작 유지(뒤로가기=페이지 이동)
  > 
  > ## 검증 (모바일 뷰포트 492px, 브라우저 실검증)
  > 
  > - /stock → /analyze → 카드 클릭(패널 열림, `history.state.aiPanelOpen=true`) → **back → 패널만 닫힘 + /analyze 유지 + marker 소비** → back 한 번 더 → /stock ✅
  > - X 닫기 → marker 소비 확인(잔여 엔트리 0) ✅
  > - `tsc --noEmit`·vitest 1305 passed·프로덕션 빌드 정상
  > 
  > ## 다음 작업
  > 
...
### 2026-07-14 — feat(intraday): 세션 상태 시인성 — 상태 필+스트라이프+일시정지 디밍 (#364)

- **slug**: `intraday-session-status` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/364
- **요약**: feat(intraday): 세션 상태 시인성 — 상태 필+스트라이프+일시정지 디밍
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경
  > 
  > /intraday 워치 표에서 세션 상태가 회색 캡션("실행 중")과 재생/일시정지 아이콘뿐이라, 어떤 세션이 돌고 있는지 훑어서 구분할 수 없었다(사용자 지적). 시안 4종 비교 후 추천안(필+스트라이프+절충 디밍) 채택.
  > 
  > ## 변경 내용
  > 
  > 1. **상태 필(StatusPill)** — 회색 캡션+경고배지를 색+모양+텍스트 삼중 부호 필로 교체
  >    - 실행 중: 녹색 필 + 맥박 점 (`market-badge-open` 장중 배지와 같은 문법·토큰)
  >    - 일시정지: 회색 필 + 정지 아이콘 (의도된 정지)
  >    - 판단 끊김(구 "멈춤"): 앰버 경고 필, hover 힌트 유지
  >    - 완료: 테두리형 조용한 필 / 실패: critical
  > 2. **좌측 상태 스트라이프** — 첫 셀 w-xs 세로 바(실행=녹색·판단 끊김=앰버·일시정지=회색). 펼침 패널에도 같은 색 연장
  > 3. **일시정지 절충 디밍** — 일시정지 행만 종목명·현재가·등락률·수익률·평가금액 muted (완료 세션은 성과 비교 대상이라 유지)
  > 4. **그룹 요약 정합** — 날짜 그룹 "실행 N" 카운트 accent-vivid → market-open
  > 5. **카피** — stalled 배지 "멈춤" → "판단 끊김" (일시정지와 의미 겹침 해소)
  > 
  > ## 설계 노트
  > 
  > - 신규 토큰 0 — 전부 기존 토큰(market-open·warn·critical 계열) 재사용, 다크모드 자동
  > - 상태 클래스는 조건부 리터럴 (속성 선택자+@apply Turbopack 누락 함정 회피)
  > - memo 행 유지 — 신규 props 없음, stripe/dimmed 는 행 내부 파생값
  > 
  > ## 다음 작업
  > 
  > - 모바일 실기기에서 스트라이프·필 시인성 확인 (mobile-perf 이니셔티브 잔여 관찰과 함께)
  > - 세션 상세(/intraday/[sessionId]) 헤더에도 동일 StatusPill 문법 적용 검토
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - 모바일 실기기에서 스트라이프·필 시인성 확인 (mobile-perf 이니셔티브 잔여 관찰과 함께)
  - 세션 상세(/intraday/[sessionId]) 헤더에도 동일 StatusPill 문법 적용 검토

### 2026-07-14 — feat(intraday): 체결 내역 판단 메모 + 기본 탭 변경 + 그리드 정렬 (#365)

- **slug**: `intraday-order-log-rationale` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/365
- **요약**: feat(intraday): 체결 내역 판단 메모 + 기본 탭 변경 + 그리드 정렬
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경
  > 
  > 단타 워치 펼침의 체결 내역이 팩트(시각·구분·수량×가격·손익)만 보여줘 "왜 샀/팔았는지"를 보려면 시트나 상세로 이동해야 했다. 또 기본 탭이 차트라 체결 확인까지 클릭이 한 번 더 필요했다(사용자 요청).
  > 
  > ## 변경 내용
  > 
  > 1. **체결별 판단 메모** — 틱 rationale 을 체결에 실어 각 행 우측에 표시. 최대 2줄 말줄임(`line-clamp-2`) + hover 전문(title). 모바일(sm 미만)은 팩트 아래 전폭 랩
  > 2. **기본 탭 변경** — 펼침 기본 탭 차트 → 체결 내역, 탭 순서 '체결 내역 | 차트'. recharts 지연 로드가 차트 탭 첫 진입까지 미뤄져 기본 펼침 경로 경량화(#355 정합)
  > 3. **보이지 않는 그리드 정렬** — ul 5트랙 grid + li `grid-cols-subgrid` 로 행마다 팩트 폭이 달라도 컬럼 시작점 일치. 손익 null 행(매수)도 빈 셀로 트랙 유지해 메모 컬럼 안 밀림
  > 
  > ## 설계 노트
  > 
  > - subgrid: 2023년부터 전 모던 브라우저 지원
  > - 판단은 틱 단위라 같은 틱의 매수/매도 체결엔 같은 메모가 붙음(의도)
  > - 차트 탭 마커 데이터(allOrders)에 rationale 필드 추가는 마커 렌더에 무영향
  > 
  > ## 다음 작업
  > 
...
### 2026-07-28 — fix(ai-analysis): enum 폴백 관측성 + 테제 무효화 배지 (#372)

- **slug**: `fix/decision-enum-normalization` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/372
- **요약**: fix(ai-analysis): enum 폴백 관측성 + 테제 무효화 배지
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > > PR #372 에 이어붙인 두 번째 커밋입니다(같은 브랜치). 첫 커밋은 enum 조용한 폴백 제거·관측성이고, 이 커밋은 아래 기능입니다.
  > 
  > ## 배경 — 조기경보가 아무도 안 보는 곳에서 울리고 있었다
  > 
  > 픽스 이후 판정 19건을 실제 일봉과 대조한 결과(2026-07-28):
  > 
  > - **약세 판정의 상방 무효화 라인이 실제로 3건 발동**했고, 그중 **2건(HMM·SK)은 발동 후 주가가 7~8% 더 올라** "이 약세 판정은 틀렸다"를 일주일 앞서 정확히 경고했습니다. (감사에서 S-Oil을 UNDERWEIGHT하고 +54% 방치했던 결함의 해소를 실증)
  > - 목표·손절 **동시 터치 0건** (구 시맨틱은 39건 중 19건이 같은 날 동시 터치)
  > 
  > 그런데 이 신호를 표면화하는 곳이 **저장 결정 상세뷰뿐**이라(`SavedDecisionView`), `/analyze` 목록에선 어떤 판정이 깨졌는지 알 수 없었습니다. 종목을 콕 집어 열어보지 않으면 영영 모르는 상태였습니다.
  > 
  > ## 변경
  > 
  > | 계층 | 내용 |
  > |---|---|
  > | `lib/stock/thesisBreach.ts` (신규) | 현재가가 무효화(약세 상방)/손절(강세 하방) 라인을 넘었는지 판정하는 **순수 함수**. 레벨 파생은 `deriveAiVerdictLevels` 재사용(차트 오버레이와 동일 기준). `decisionStaleness`의 "근접 3%"와 달리 **실제 돌파**만 좁게 봄(목록 배지 오탐 억제) |
  > | `decisionStore.getDecisionThesisLevels` | 배지에 필요한 4개 필드만 뽑는 **경량 JSONB projection**. 카드 RPC를 바꿔 SQL 마이그레이션을 유발하지 않음 |
  > | decisions 라우트 | 배치 시세(`intstock_multprice` 1콜) + 레벨 합성. **전 구간 fail-soft** — 2.5s 예산, KIS 미설정/비-prod 스킵, 실패 시 배지만 빠지고 목록은 정상 |
  > | `ThesisBreachBadge` + 카피 + 카드 배선 | 경고(warn) 토큰 배지, title에 라인 가격·초과폭. 토큰만 사용(hex·`dark:` 클래스 0) |
  > 
  > ## 검증
  > 
  > - **라이브 end-to-end**: 라우트 핸들러 직접 호출 → **200**, 실데이터 20건 중 **삼성전자 OVERWEIGHT가 손절선 262,500을 16.2% 아래로 이탈**해 배지 대상으로 잡힘. 응답 페이로드에 `thesisBreach` 정상 포함
  > - 회귀 테스트 9종 신규 / 전체 **1315 테스트 통과**, 타입체크·ESLint 클린
  > 
  > ## 알려진 한계 (의도)
  > 
  > 배지는 **현재 상태**만 봅니다. HMM·SK처럼 돌파 후 라인 아래로 되돌아온 판정은 배지가 사라집니다 — 과거 돌파 이력을 보려면 종목별 일봉 경로가 필요한데 목록 엔드포인트 예산을 넘습니다. "지금 깨져 있는 판정"을 잡는 용도로 한정됩니다.
  > 
  > ## 다음 작업
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - 사후검증에서 함께 나온 개선 후보: ①HOLD 판정의 target/stop 세맨틱(크래프톤이 목표·손절 둘 다 터치) ②무효화 폭 편차(한국전력 +13%로 사실상 발동 불가 — 프롬프트 가이드는 +4~8%) ③강세 콜 성적 부진(1/3, 표본 작음)
  - confidence·time_horizon 상수 문제는 첫 커밋 PR 본문 참조(미해결, 결정론 신호 기반 산출 검토 권고)

### 2026-07-28 — feat(analyze): 카드에 채점 결과 표시 — '이 판단이 맞았나' (#373)

- **slug**: `feat/analyze-outcome` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/373
- **요약**: feat(analyze): 카드에 채점 결과 표시 — '이 판단이 맞았나'
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경 — 피드백 루프의 마지막 고리
  > 
  > `signal_scorecard`는 판정마다 d1/w1/w2/m1 네 시점의 적중을 쌓습니다. **#347로 채점이 복구된 뒤 매일 정상 축적 중**입니다(현재 82행, 카드 20건 중 14건이 이미 채점 완료).
  > 
  > 그런데 그 결과를 보려면 `/dashboard/scorecard` 집계로 가야 했습니다. 정작 **판정을 읽는 자리**인 `/analyze` 카드에는 피드백이 0이라, "이 판단이 맞았나"를 알 수 없었습니다. 앞선 PR #372가 "지금 이 판정이 깨졌나"(무효화 배지)를 붙였다면, 이건 "지난 판정이 맞았나"입니다 — 같은 카드에서 짝을 이룹니다.
  > 
  > ## 변경
  > 
  > | 계층 | 내용 |
  > |---|---|
  > | `lib/stock/decisionOutcome.ts` (신규, 순수) | 카드(종목당 최신 1건, upsert) ↔ 원장(실행마다 append) **매칭을 시각 근접으로** 해결. 48h 창 밖은 다른 실행으로 보고 제외(오귀속 방지). 표시는 **가장 성숙한 채점 완료 시점 1건**만(m1>w2>w1>d1) — 네 시점을 다 보여주면 카드가 시끄러움. `skipped`(봉 부재)는 판정 성패가 아니라 데이터 사정이라 제외 |
  > | `scorecardStore.getRecentOutcomeRows` | 해당 종목·필요 컬럼만 뽑는 경량 조회. 집계용 `getAllScorecardRows`(2,000행 전 컬럼)는 목록 엔드포인트에 과함. fail-soft |
  > | decisions 라우트 | outcome 합성. 예산 1.5s + catch warn 로그 — 실패해도 목록은 정상 |
  > | 카드 메타라인 | `1주 적중` / `2주 빗나감` 표기. **방향색(강세 빨강/약세 파랑)과 혼동되지 않게 방향 무관 톤** 사용(적중=accent, 빗나감=warn). title에 초과수익(%p) 병기 + "자가 채점·초과수익 기준" 명시 |
  > 
  > ## 검증
  > 
  > - **라이브 end-to-end**: 라우트 핸들러 직접 호출 200, 카드 20건 중 **14건에 결과 부착**
  >   - SK하이닉스 `w1 적중`(exc −7.2%p) · SK `w2 빗나감`(exc +6.7%p) · 아리바이오랩 `w2 적중`(exc −13.4%p)
  >   - 삼성전자는 `w1 보합` + **손절이탈 배지 동시 표시**(두 기능이 함께 동작)
  > - 회귀 테스트 8종 신규 / 전체 **1324 테스트 통과**, 타입체크·ESLint 클린
  > - SQL 마이그레이션 불필요(기존 컬럼만 사용)
  > 
  > ## 설계 메모
  > 
  > - 결과 기준은 **초과수익(벤치 지수 대비)**이라 "시장 대비" 성패입니다. 절대수익으로 보면 하락장에서 약세 판정이 과대평가되므로(감사에서 절대 61% vs excess 44%로 확인), title에 기준을 명시했습니다.
  > - `flat`(±기준 밴드 안)은 적중/빗나감 어느 쪽도 아니라 "보합"으로 별도 표기 — 집계 분모에서도 빠지는 값입니다.
  > 
  > ## 다음 작업
  > 
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - **phase-2 터치 기반 채점** — target/무효화 터치를 cron이 매일 기록. 선행조건(#350 반대 방향 밴드)이 충족됐고, PR #372 배지의 "현재 상태만 본다"는 한계도 함께 해소됨(과거 돌파 이력이 영구 기록). **신규 컬럼 필요 → SQL 수동 실행**
  - 폴링(30s/15s)마다 KIS 1콜 — 짧은 서버 캐시 또는 장중 게이팅 검토(#372 후속)
  - HOLD 판정의 target/stop 세맨틱(구조 수정 성격, 빈도 낮아 후순위)

### 2026-07-28 — feat(scorecard): phase-2 터치 채점 — 목표·무효화 라인 실제 도달 기록 (#374)

- **slug**: `feat/scorecard-touch-scoring` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/374
- **요약**: feat(scorecard): phase-2 터치 채점 — 목표·무효화 라인 실제 도달 기록
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > > **선행 조건 충족** — `docs/sql/scorecard-touch-scoring.sql` 은 prod Supabase에 **이미 적용 완료**(프로젝트 관례대로 코드 머지 전 수동 선적용).
  > 
  > ## 배경
  > 
  > phase-1 채점은 horizon(d1/w1/w2/m1)의 **종가 방향**만 봅니다. 정작 판정이 스스로 내건 검증 가능한 약속 — **목표가·무효화 라인** — 은 저장만 하고 채점에 쓰지 않았습니다.
  > 
  > **#350이 이걸 가능하게 만들었습니다.** 약세 콜의 target(하방 재진입)과 stop(상방 무효화)이 **반대 방향 밴드**가 되면서 비로소 "무엇이 먼저 닿았나"가 판별력을 갖습니다. 구 시맨틱은 둘 다 하방이라 39건 중 **19건(49%)이 같은 날 동시 터치**였습니다.
  > 
  > 또한 이건 PR #372 무효화 배지의 **"현재 상태만 본다"는 한계**를 해소합니다 — 돌파 후 되돌아온 판정도 "그날 깨졌다"가 영구 기록으로 남습니다.
  > 
  > ## 변경
  > 
  > | 계층 | 내용 |
  > |---|---|
  > | `touchScoring.ts` (신규, 순수) | **부호가 방향**(target≥0·stop>0 은 고가, 음수는 저가) — `thesisBreach`·차트 오버레이와 동일 기준. 증분 커서로 멱등, **최초 터치만** 보존, 창(45일=m1+여유) 종료 후 스캔 중단. entry 당일 봉 제외(진입가가 라인에 닿아 보이는 오판정 방지) |
  > | `levelsOnlyUpdate` | 레벨은 일봉 없이도 산출 가능 → **당일 진입 행도 즉시** target_price/stop_price 를 채운다(커서는 전진 안 함) |
  > | `scorecardStore` | 스캔 대상 조회(미스캔 우선 + **최신 판정 우선** — 활성 구간이 배치를 먼저 차지) + 부분 갱신 |
  > | `runTouchScan` | ticker 단위 격리. **실패 시 커서를 전진시키지 않아** 다음 실행이 같은 구간을 재시도(누락 대신 재시도) |
  > | cron | `flow-snapshot` 단계 ④로 배선(독립 try/catch — 다른 단계와 무관하게 실행) |
  > 
  > ## 라이브 검증 (실제 기록됨)
  > 
  > 실행 결과 `scanned:35, targetHits:23, stopHits:21, errors:0`. **신규 시맨틱 14행**의 터치 선후:
  > 
  > | 종목 | 판정 | 목표/재진입 | 무효화/손절 | 선후 |
  > |---|---|---|---|---|
  > | 052900 | UNDERWEIGHT | 1,108 → 07-20 | 1,300 → **07-15** | **무효화 먼저** |
  > | 034730 | UNDERWEIGHT | 541,000 → 07-28 | 629,000 → **07-22** | **무효화 먼저** |
  > | 259960 | HOLD | 259,000 → 07-27 | 219,000 → **07-20** | 손절 먼저 |
  > | 011200 | UNDERWEIGHT | 미터치 | 20,500 → 07-16 | 무효화만 |
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - **터치 데이터를 표면화** — 현재는 원장에만 쌓입니다. /analyze 카드나 스코어카드 대시보드에 "무효화 07-22 돌파" 같은 이력을 노출하면 PR #372 배지의 현재상태 한계가 완전히 해소됩니다
  - 터치 기반 **집계 지표**(무효화 발동률·목표 도달률·선후 분포)를 `/dashboard/scorecard` 에 추가 검토
  - 감사 잔여: HOLD 판정의 target/stop 세맨틱(크래프톤이 목표·손절 둘 다 터치 — 이번 데이터로 재확인됨), 무효화 폭 편차(한국전력 +13% 미발동)

### 2026-07-28 — fix(ai-analysis): 가격 레벨 실측값 주입 + 일봉 1년 창 + 재진입가 실측 지지 앵커 (#375)

- **slug**: `feat/price-levels-context` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/375
- **요약**: fix(ai-analysis): 가격 레벨 실측값 주입 + 일봉 1년 창 + 재진입가 실측 지지 앵커
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경 — 무엇이 문제였나
  > 
  > AI 종합분석이 받던 기술 데이터는 축 점수와 지표 **값**(%B, ADX)뿐이라, "볼린저 하단을 터치했다"는 사실은 알아도 **그 하단이 얼마인지는 몰랐다**. 그 결과 모델이 레벨을 추정해 서술했다.
  > 
  > - 실측: 삼성전자 분석이 **"233,000원(20일선)"** 이라 했으나 실제 20일선은 **276,350원** (18% 오차)
  > - 반면 데이터로 주어진 값(52주 저가 등)은 정확 → **정보 부재가 원인이지 모델 능력 문제가 아님**
  > 
  > 또한 사용자 관점의 핵심 판단 — "현재가가 여러 매물대 중 하나를 깨고 내려왔는지, 아래에 다음 매물대가 남았는지" — 이 아예 입력에 없었다.
  > 
  > ## 무엇을 했나
  > 
  > ### 1. 가격 레벨 실측값 주입 (`lib/signal/levels/`)
  > 
  > - `computePriceLevels` — 이동평균(5/20/60/120) **실제 가격**, 볼린저(20,2) 상/중/하단 가격, 직전 상승 파동 + 피보나치 되돌림, 매물대(최근 1년 거래량 가격분포)
  > - 매물대를 현재가 기준 **위(저항) / 도달(지지 시도) / 아래(다음 지지 후보)** 로 분류
  > - `formatPriceLevelsForPrompt` — 해석 규칙까지 함께 주입: "매물대를 깨고 내려왔다면 아래 첫 매물대까지 **추가 하락 여지**, 도달·근접했다면 **지지 시도 구간**, 위쪽 매물대 돌파는 **추세 전환 트리거**"
  > - route 에는 **fail-soft** 로 붙인다(계산 실패 시 레벨 없이 진행 — 무회귀)
  > 
  > ### 2. 일봉 창 200일 → 365일
  > 
  > 200일 창(≈134봉)이 반년 전 매물대를 통째로 놓쳐 **"아래 지지 공백"으로 오판**하고 있었다. 두산에너빌리티 실측 대조:
  > 
  > | | 200일 창 | 365일 창 |
  > |---|---|---|
  > | 현재가 도달 매물대 | **없음** | 63,948원 (비중 11.6%) |
  > | 아래 지지 후보 | **"없음 — 공백"** | 60,277원 (6.2%) |
  > | 피보나치 | **미생성** (창 안 최저가라 되돌릴 파동 없음) | 전체 (되돌림 86%) |
  > | 봉 수 | 134 (120일선·limitedData 임계 턱걸이) | 244 |
  > 
  > 원시 봉은 프롬프트에 실리지 않고 파생 지표로만 요약되므로 토큰 증가는 미미하다.
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - 여러 종목 표본으로 `target_pct` 앵커 규칙 준수율 관측 — 규칙을 무시하는 빈도가 유의미하면 프롬프트가 아닌 **후처리 검증**(공백 구간이면 서버에서 null 강제)으로 승격
  - 급등 종목의 원거리 매물대 처리(거리 상한 도입 여부) 판단
  - `confidence`·`time_horizon` 이 전 종목 상수(MEDIUM/중기)로 나오는 문제 — 프롬프트 튜닝이 이미 무효로 확인됐으므로 **결정론적 계산**으로 전환 검토

### 2026-07-30 — fix(ai-analysis): PM 타임아웃 상한을 실측 분포에 맞춰 상향 (480s → 900s) (#376)

- **slug**: `fix/pm-timeout-headroom` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/376
- **요약**: fix(ai-analysis): PM 타임아웃 상한을 실측 분포에 맞춰 상향 (480s → 900s)
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 증상
  > 
  > 한미약품 분석이 **계속 '응답 시간 초과'로 실패**. 확인해 보니 한미약품만의 문제가 아니고, **최종 단계(portfolio_manager)에서만** 죽어 **12분 걸린 분석 전체가 버려지는** 실행이 반복 관측됐다.
  > 
  > `ai_agent_usage` 기록(에이전트별 성공 시에만 행이 남음) — 한미약품 3회 시도:
  > 
  > | run | 시각(UTC) | 완료 단계 | PM |
  > |---|---|---|---|
  > | 529e97f7 | 07-28 09:32 | 14 | 260.7s 성공 |
  > | 5a0db213 | 07-29 02:50 | 13 | **없음 = 실패** |
  > | d30ee3f4 | 07-29 23:34 | 13 | **없음 = 실패** |
  > 
  > 같은 패턴이 셀트리온·SK하이닉스·036570 에서도 나온다.
  > 
  > ## 원인 — 상한은 그대로인데 실측 분포가 밀렸다
  > 
  > `T.PM = 480_000` 의 코드 주석에 설정 근거가 남아 있었다: *"실측 성공 139~224초"*. 지금 실측(n=115):
  > 
  > | 기간 | n | 중앙값 | 최대 |
  > |---|---|---|---|
  > | ~7/19 | 93 | 155s | 310s |
  > | **7/20~** | 22 | **261s (+68%)** | **405s** |
  > 
  > 480초까지 남은 여유가 **75초(18%)** 뿐이다. 그 사이 PM 프롬프트에 무효화 라인(#350)·캘리브레이션 4규칙(#352)·가격 레벨 블록과 재진입 앵커 규칙(#375)이 누적됐다. **PM 이 읽고 판단할 게 늘어난 만큼 시간도 늘었는데 뚜껑만 그대로였다.**
  > 
  > 검증한 반증 두 가지:
  > - **#375 때문이 아니다** — 머지 전인 07-28 09:15(036570)에도 같은 실패가 있었고, #375 이후 PM 은 283s·301s 로 정상 범위다.
  > - **동시 실행 부하 때문도 아니다** — 겹침 수와 실패율에 상관이 없었다(겹침 0 에서도 실패, 겹침 4 에서 전부 성공).
  > 
  > ## 구조적 원인 — 슬랙이 0이었다
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - PM 소요시간 추세 관측 — 900초도 잠식되면 프롬프트 다이어트가 필요하다는 신호
  - `confidence`·`time_horizon` 이 전 종목 상수인 문제를 결정론 계산으로 전환(프롬프트 튜닝은 2회 실패 확인) — PM 부담을 줄이는 방향과도 맞물린다

### 2026-07-30 — feat(ai-analysis): 약세 재진입가를 실측 매물대에 서버가 강제 앵커 (#377)

- **slug**: `feat/reentry-anchor-enforcement` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/377
- **요약**: feat(ai-analysis): 약세 재진입가를 실측 매물대에 서버가 강제 앵커
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경
  > 
  > #375 에서 프롬프트 규칙으로 재진입가 모순을 막았지만, 그 PR 이 스스로 남긴 한계가 있다:
  > 
  > > 프롬프트 규칙은 **행동 유도이지 강제가 아니다**. 모델이 규칙을 무시하면 다시 임의 값이 나올 수 있고, 이는 사후에만 검출된다.
  > 
  > 이 숫자는 단순 텍스트가 아니라 **차트 오버레이의 재진입 라벨과 phase-2 터치 채점을 구동**한다. 규칙이 한 번 무시되면 아래에 받쳐줄 매물대가 없는 위험한 가격이 권위 있는 숫자로 화면에 찍힌다(실측: 두산에너빌리티 −13% = 55,400원 — 매물대 공백 한복판).
  > 
  > ## 변경
  > 
  > - **`checkReentryAnchor` 신설**(순수 함수) — 약세(UNDERWEIGHT/REDUCE)의 재진입 예상가가 현재가 이하 매물대 **구간 안**에 있는지 검증. 아니면 `target_pct` 를 null 로 강제하고 `target_pct_voided_reason` 에 사유 기록(JSONB, 마이그레이션 불필요).
  > - **허용 오차 = 매물대 구간폭의 절반.** 고정 %가 아니다 — 구간폭은 종목의 1년 가격 범위에서 나오므로 변동성에 자동 스케일한다(실측: HMM 379원 vs 삼성전기 95,000원). 고정 ±3% 는 저변동주에서 헐겁고 고변동주에서 빡빡하다.
  > - **강세는 검증하지 않는다.** 비대칭이 실재한다 — 현재가 아래 공백은 "받아줄 매수 평단이 없다"(낙폭 확대)지만, 위쪽 공백은 "팔 물량이 없다"(오히려 잘 오름). 상방 목표가 저항 위에 있는 건 모순이 아니다.
  > - `PriceLevels.binWidth` 노출. route 는 프롬프트에 주입한 **같은 레벨 객체**를 검증에 재사용 — KIS 추가 조회 0건.
  > 
  > ### 프롬프트–서버 규칙 불일치 교정
  > 
  > dry-run 중 발견했다. 기존 프롬프트 문구는 *"아래에 받쳐줄 매물대가 **없으면** null"* 이라 **총공백만** 다루는데, 서버 규칙은 *"매물대 **안에** 있어야 한다"* 는 더 엄격한 조건이었다. 이대로면 PM 이 프롬프트를 지켜도 서버가 무효화해 **준수율 측정이 오염**된다.
  > 
  > 어느 쪽이 맞나 — 사용자가 정의한 논리는 *"아직 매물대가 더 아래에 있으면 그 가격까지 하락 위험이 있는 것이고, 이미 지지대 근처면 매수 타이밍"*. 삼성전기 사례(재진입가 1,002,800원인데 실제 매물대는 944,500원 = 아직 5.8% 하락 여지)가 정확히 이 케이스라 **서버가 맞고 프롬프트가 느슨했다.** 프롬프트를 교정했다.
  > 
  > ## 검증 — 라이브 3종목, 실측 1:1 대조
  > 
  > | 종목 | 실측 지지 매물대 | 모델이 낸 재진입가 | |
  > |---|---|---|---|
  > | 삼성전기 | 944,500원 (비중 3.6%) | −4.7% = **944,423원** (77원 차) | 앵커 ✓ |
  > | HMM | 20,372원 (비중 13.6%) | −3.5% = **20,362원**, 본문에 "20,372원, 매물대 비중 13.6%" 명시 | 앵커 ✓ |
  > | 한미약품 | **없음 — 완전 공백** | **null** + "353,927원 거래량 동반 돌파 시 추세 전환"(실측 저항 정확) | 폴백 ✓ |
  > 
  > 개선 전 같은 종목: 삼성전기 −8% = 1,002,800원(매물대 아님) / HMM −8% = 17,968원(지지를 깨고 내려간 공백).
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - `target_pct_voided_reason` 발동률 관측 — 잦으면 카드에 "재진입가 미제시 — 아래 지지 공백" 표기 추가
  - 원거리 매물대 상한 도입 여부 판단(표본 축적 후)
  - **목표·무효화 라인이 노이즈 안에 있는 문제** — 별도 분석에서 확인됐다: 제시 밴드 중앙값 9.0%/8.0% vs 2주 실제 변동 중앙값 8.8%. 터치 채점 82건 중 47건(57%)이 두 라인을 모두 터치해 변별력이 없다. ATR 기반 변동성 스케일링 검토 필요(`lib/signal/levels/atr.ts` 기존 자산)

### 2026-07-30 — feat(ai-analysis): 목표·무효화 라인을 종목 변동성에 맞춘다 — 통상 변동폭 주입 (#378)

- **slug**: `feat/volatility-scaled-levels` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/378
- **요약**: feat(ai-analysis): 목표·무효화 라인을 종목 변동성에 맞춘다 — 통상 변동폭 주입
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 문제
  > 
  > 프롬프트가 모든 종목에 **고정 폭**(약세 무효화 +4~8%, 강세 손절 -5~8%)을 쓰는데, 종목별 **통상 2주 변동폭은 2.4%~13.3% 로 5배 넘게 갈린다**(실측 80건).
  > 
  > 그 결과 변동성 큰 종목일수록 무효화 라인이 노이즈에 잠긴다 — 논거가 깨져서가 아니라 **평소 움직임만으로 발동**한다:
  > 
  > | 종목 | 통상 2주 변동 | 무효화폭 | |
  > |---|---|---|---|
  > | SK하이닉스 | ±14.6% | ±8.0% | **0.55배 — 노이즈 안** |
  > | 삼성전기 | ±11.8% | ±7.0% | **0.59배 — 노이즈 안** |
  > | 두산에너빌리티 | ±7.5% | ±6.2% | **0.83배 — 노이즈 안** |
  > 
  > 반대로 제일기획(통상 ±2.6%)에는 고정 4~8% 가 통상 변동폭의 1.5~3배라 사실상 발동하지 않는다. 터치 채점 82건 중 47건(57%)이 목표·무효화를 둘 다 터치해 두 라인이 변별력을 잃은 것도 같은 뿌리다.
  > 
  > ## 변경
  > 
  > - **`PriceLevels.typicalMove` 신설** — 1주/2주/1개월 |수익률| 중앙값을 과거 1년 겹치는 창에서 **직접** 측정해 프롬프트 레벨 블록에 주입.
  > - **프롬프트의 고정 % 가이드를 변동성 상대 기준으로 교체.** 통상 변동폭보다 좁게 잡으려면 왜 노이즈가 아닌 구조적 경계인지(매물대·직전 저점) reasoning 에 근거를 대게 한다.
  > - **`checkStopNoiseBand`** — 라인이 통상 변동폭 안이면 **기록만** 한다(`stop_noise_warning`, 강제 안 함). 매물대 같은 구조적 경계면 좁아도 정당할 수 있어서다. 재진입 앵커(#377)와 같은 "계측 먼저, 필요하면 승격" 패턴.
  > 
  > ### ATR×√N 은 검증에서 탈락시켰다
  > 
  > | 공식 | 실측 2주 변동 / 예측 |
  > |---|---|
  > | 일간 ATR% × √10 | **0.29배** (3.4배 과대추정) |
  > | **기간 맞춰 직접 측정** | **1.30배** ← 채택 |
  > 
  > ATR 은 변동성 급등 직후 과거를 보느라 부푼다(삼성전자 7/29 일중 범위 21%, 일간 ATR 11.55%). √10 배로 늘리면 2주 기대 변동폭이 28.9% 가 되어, 그 밖에 라인을 두라고 하면 **영원히 발동하지 않는다.** `lib/signal/levels/atr.ts` 를 쓰려던 초기 계획을 보정 실패로 폐기했다.
  > 
  > ## 검증 — 라이브 9종목
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - 동시 터치율 추이 관측 — 57% 에서 내려가는지가 이 변경의 실제 성패
  - 배수 상한 필요 여부 판단(과도하게 넓은 라인)
  - **signal_score 역상관 검증용 표본 보충 진행 중** — 고득점 종목을 겨냥해 7건 추가(60점 이상 12건 → 20건, 70점 이상 1건 → 4건). w1 채점은 5영업일 뒤

### 2026-07-31 — refactor(copy): '무효화' → '재검토' — 뜻이 바로 읽히는 말로 교체 + 라벨 단일화 (#379)

- **slug**: `chore/rename-invalidation-copy` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/379
- **요약**: refactor(copy): '무효화' → '재검토' — 뜻이 바로 읽히는 말로 교체 + 라벨 단일화
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경
  > 
  > 차트의 **"무효화 라인"**이 한 번에 이해되지 않는다는 피드백. **무엇이** 무효가 되는지가 드러나지 않는 게 원인이다.
  > 
  > 이 선의 실제 의미는 *"여기까지 오르면 약세로 본 근거가 깨지므로 판단을 다시 봐야 하는 가격"* 이라, 화면에서는 **"재검토 가격"** 으로 부른다. 강세 쪽 **"손절"** 은 익숙한 말이라 그대로 둔다.
  > 
  > ## 화면 문구 변경 (5곳)
  > 
  > | 위치 | 전 | 후 |
  > |---|---|---|
  > | 차트 우측 축 알약 | `무효화 353,927` | `재검토 353,927` |
  > | AI 패널 레벨 | `무효화` | `재검토 가격` |
  > | /analyze 카드 배지 | `무효화` | `재검토 필요` |
  > | 배지 마우스오버 | "무효화 라인 … 약세 판단 근거가 깨졌어요. 재분석을 권해요." | "재검토 가격 … 약세로 본 근거가 깨졌어요. 다시 분석해 보세요." |
  > | 재분석 배너 | `무효화 라인 부근이에요` | `재검토 가격 부근이에요` |
  > | 단타 판독 필드 | `무효화가` | `재검토가` |
  > 
  > 차트 알약은 3자로 길이가 같아 레이아웃 영향이 없다.
  > 
  > ## 라벨 단일화
  > 
  > 같은 `role → 라벨` 매핑이 **차트 축 라벨 2곳 + 판정 스트립**에 하드코딩돼 있었다. 문구를 바꾸는 김에 `AI_LEVEL_ROLE_LABEL` 로 copy 계층에 모은다 — 다음에 문구가 갈라지지 않는다.
  > 
  > 내부 role 명 `invalidation` 은 **도메인 개념어라 유지**한다. AGENTS.md 상 UI 문구는 `lib/copy/` 가 담당하므로, 개념어와 표시어를 분리하는 게 구조에 맞다.
  > 
  > ## PM 프롬프트 용어도 맞췄다
  > 
  > 프롬프트가 이 레벨을 "무효화" 라 부르면 **모델이 `short_term_outlook` 등 사용자에게 보이는 텍스트에 그대로 써서** 화면 표기와 어긋난다. 서술형 텍스트에서는 "재검토 가격" 을 쓰도록 명시했다.
  > 
  > ## 이름 충돌 해소
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - 하영님 브라우저에서 차트 알약·카드 배지 육안 확인 (자가 PR 이라 실질 오버사이트가 사용자 확인)

### 2026-08-03 — feat(intraday): AI 단타 판단 고도화 1주차 — 선행 정보 주입 + conviction 천장 제거 + 관측 창 (#380)

- **slug**: `hylee/intraday-improvement-week1` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/380
- **요약**: feat(intraday): AI 단타 판단 고도화 1주차 — 선행 정보 주입 + conviction 천장 제거 + 관측 창
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 무엇을
  > 
  > AI 단타 판단의 **신뢰도 개선 1주차** — 선행 정보 주입(I1~I3), conviction 인공 천장 제거, 관측 가능성 확보. 라이브 3거래일 검증 완료.
  > 
  > > ⚠️ **수익성을 주장하지 않는다.** 이 PR의 범위는 **판단 품질의 구조적 결함 제거 + 측정 가능하게 만들기**다. 수익 판정은 20~30 라운드트립 누적 후(현재 21건).
  > 
  > ## 왜
  > 
  > 기존 judge 는 942회 실질 100% HOLD → 컷을 65로 열었더니 이번엔 **conviction 이 62-64 에 47% 밀집하고 65를 못 넘는 인공 천장**이 생겨 실매매가 사실상 0이었다. 원인은 프롬프트가 `높은 확신(65↑)` 으로 **컷 숫자를 못 박은 것**(코드 컷은 58인데 LLM 은 65 기준으로 판단 = 서로 다른 기준).
  > 
  > ## 핵심 변경
  > 
  > | # | 변경 | 효과(실측) |
  > |---|---|---|
  > | I1 | 일봉 흐름 컨텍스트 주입(MA 배열·MACD 교차·RSI·20일 위치) | rationale 에 “역배열 일봉이라 A+ 아님” 등 반영 |
  > | I2 | A+ 셋업 우선순위 + 거부권 + 충돌 해소 규칙 | “절반 좋고 절반 나쁨”을 관망으로 정직 판정 |
  > | I3 | 수급 선행(체결강도·호가 잔량) judge 주입 | “수급 확인/확인 실패” 로 확신 조정 |
  > | — | **밴드 재배치 + 컷 숫자 제거** (A+ 75~89 · 단일근거 60~74 신설) | 62-64 **25%→2%**, 최대 **65→80**, A+ 밴드 **첫 사용** |
  > | — | BUY 컷 65→58 | 실매매 **0 → 21 라운드트립** |
  > | — | **약세 레짐 관측 창** — 사전 게이트만 개방 | AI 호출률 35%→**73%**. **사후 레짐 veto 무변경**이라 하락장 진입 위험 불변 |
  > | — | 오답노트 규칙 문구에서 코드 컷 참조 제거 | 컷을 바꿔도 규칙 의미가 안 변함 |
  > | — | 오토파일럿 신규 fill 마감 14:00→15:00 | 늦게 시작한 날도 종목 로테이션(하루 33종목 순회) |
  > | — | `scripts/intraday/today.mts` 신규 | 일일 실황 + **비용 분해** + 청산 사유 한 줄 조회 |
  > 
  > ## 검증
  > 
  > QA 리포트: `docs/qa/intraday-improvement-week1.md` (AC 10항목 전부 PASS)
  > 
  > - 전체 **1432 테스트 통과** / tsc 0 / 변경 파일 lint 0 에러 / 프로덕션 빌드 성공
  > - **라이브 3거래일**(7/29·30·31) 실장 운영 — 런타임 에러 **0건**(틱 344·405·290)
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - [ ] 실매매 **20~30 라운드트립**까지 누적 후 수익성 판정 — 기준은 **순수 가격손익이 왕복비용 0.28%를 넘는가**. 매일 `npx tsx scripts/intraday/today.mts` 로 비용 분해·청산 사유 기록.
...
### 2026-08-23 — docs(signal): 신호 토대 감사 — 4축은 비용을 넘는 예측력이 없음, 단타 자동매매 트랙 종료 (#383)

- **slug**: `hylee/signal-edge-audit` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/383
- **요약**: docs(signal): 신호 토대 감사 — 4축은 비용을 넘는 예측력이 없음, 단타 자동매매 트랙 종료
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 무엇을
  > 
  > `lib/signal/` 4축 신호에 **예측력이 있는지** 오프라인으로 감사하고, 그 결과에 따라 **단타·신호 자동매매 트랙을 종료**한다.
  > 
  > 코드 동작 변경 **0건** — 감사 스크립트·리포트만 추가한다. 라이브는 전 기간 동결 상태였다.
  > 
  > ## 왜
  > 
  > Week1 개선(#380) 후 5거래일 실매매 결과: 실현 −192,618원 · 승률 26% · **가격손익 −0.117% vs 비용 0.140%**.
  > 비용을 0으로 해도 지는 구조라, 컷·프롬프트 층이 아니라 **신호층**을 의심할 근거가 됐다.
  > 
  > ## 감사 결과 — 네 번의 독립 검증이 같은 답
  > 
  > | 검증 | 유니버스 | 타임프레임 | 표본 | 결과 |
  > |---|---|---|---|---|
  > | 분봉 감사 | 모멘텀 스크리닝 | 5·15·30·60분 | 20,354관측·27일 | 일자내 IC **−0.176**, 상위십분위 **−0.269%** |
  > | 분봉 OOS | 동일 | 동일 | 49,087관측·21일 | 역방향 재현(IC −0.110, 86% 날 일치) |
  > | 경로 검증 | 동일 | 60분 | 49,059진입 | 배리어 적용 **−0.337%**, 무선별 대조군도 −0.370% |
  > | **일봉 재감사** | **KOSPI 828** | **3·5·10·20일** | **93,136관측·356일** | **정·역 양방향 전부 미달** |
  > 
  > ### ★핵심 발견 1 — 풀링 통계가 몇 주간 판단을 오염시켰다
  > 
  > `signalScore` 풀링 IC **+0.052**(양수!)인데 **일자내 IC −0.176**, 양수인 날 19%.
  > Simpson's paradox — "좋은 날엔 점수도 수익도 높다"는 **날짜 효과**였다.
  > *"sigSpear는 양수인데 왜 잃지?"* 라는 역설의 정체가 이것이었다.
  > 
  > ### ★핵심 발견 2 — 신호는 과거는 정확히 재는데 미래를 예측하지 않는다
  > 
  > | 십분위 | 평균 점수 | ←과거 20일 | 미래 20일→ |
  > |---|---|---|---|
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - [ ] **재시도 금지 목록 준수** — 컷 튜닝·프롬프트 개선·손절 확대·물타기·타임프레임 상향·역방향 매매는 전부 검증 후 기각됨. 근거 없이 재시도하지 않는다.
  - [ ] 재검토는 **새 정보원이 오프라인 IC 감사를 통과할 때만** — 체결 미시구조·수급(외국인·기관·프로그램)·공시 이벤트·업종 상대강도. 반드시 **구현 전에** `edge-audit` 로 먼저 거른다.
  - [ ] 순서를 **"입력 후보 → 오프라인 감사 → 통과분만 구현"** 으로 고정. "구현 → 라이브 → 손실 → 분석"을 반복하지 않는다.
  - [ ] 페이퍼트레이딩·오토파일럿 코드는 관측 인프라로 존치(추가 운영 계획 없음). 제거 필요 시 별도 판단.

### 2026-08-23 — docs(analysis): 종료 기록 라이브 최종 집계 — 64 라운드트립, 후반 개선은 유의하지 않음 (#384)

- **slug**: `hylee/closeout-final-tally` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/384
- **요약**: docs(analysis): 종료 기록 라이브 최종 집계 — 64 라운드트립, 후반 개선은 유의하지 않음
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 무엇을
  > 
  > 종료 기록(`intraday-track-closeout.md`)에 **라이브 최종 집계**를 추가한다. 문서만 변경(코드 0건).
  > 
  > 종료 결정(#383) 시점에는 27 라운드트립까지만 집계돼 있었다. 이후 미분석분(8/11·13·18·21)을 마저 돌려 **하영 계정 9거래일 전량 64 라운드트립**을 확정했다.
  > 
  > ## 결과
  > 
  > | | 라운드트립 | 실현 | 가격손익 | 비용 | 승률 |
  > |---|---|---|---|---|---|
  > | 전반 7/29~8/7 | 27 | −195,602원 | −0.221% | 0.140% | 19% |
  > | 후반 8/10~21 | 37 | −5,561원 | **+0.133%** | 0.140% | 38% |
  > | **전체** | **64** | **−201,163원** | **−0.005%** | 0.140% | 30% |
  > 
  > ## ★후반 개선은 유의하지 않다
  > 
  > 거래당 가격수익률(비용 제외):
  ...
  > | | n | 평균 | SE | 95% CI |
  > |---|---|---|---|---|
  > | 전반 | 27 | −0.536% | ±0.351% | [−1.223, +0.152] |
  > | 후반 | 37 | +0.168% | ±0.368% | [−0.554, +0.890] |
  > 
  > **차이 +0.704%p ± 0.509, t = 1.38** → 유의하지 않음. 분할 자체도 사후(post-hoc) 구분이다.
  > 
  > ## ★라이브만으로는 애초에 판정 불가
  > 
  > 거래당 표준편차 **2.09%** vs 찾는 엣지 **0.28%** — 신호 대 잡음 **1:7.5**.
  > `t=3` 판정에 **약 540 라운드트립 ≈ 70 거래일(3.5개월)** 이 필요하다.
  > 
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - [ ] 종료 상태 유지 — 재시도 금지 목록(컷 튜닝·프롬프트 개선·손절 확대·물타기·타임프레임 상향·역방향 매매) 준수.
  - [ ] 재검토는 §6 조건 충족 시에만 — 새 정보원이 **구현 전** `edge-audit` 오프라인 감사를 통과하거나, 비용 구조(제세금 0.15% 하한)가 바뀔 때.
  - [ ] 향후 어떤 아이디어든 **라이브 손익으로 판정하지 않는다** — 신호대잡음 1:7.5라 수개월이 필요하다. 오프라인 감사가 1차 관문.

### 2026-08-23 — docs(analysis): 잔여 구멍 점검 — 1~2일·변동성 조건부 모두 기각, 고변동 회피 규칙 확보 (#385)

- **slug**: `hylee/closeout-vol-addendum` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/385
- **요약**: docs(analysis): 잔여 구멍 점검 — 1~2일·변동성 조건부 모두 기각, 고변동 회피 규칙 확보
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 무엇을
  > 
  > 종료 기록에 **§5-1 잔여 구멍 점검**을 추가한다. 문서만 변경(코드 0건).
  > 
  > *"지금까지 지표로 보면 더 개선 포인트가 있나?"* 에 추측으로 답하지 않고, **아직 측정하지 않은 구간 두 개를 실제로 쟀다.** 일봉 픽스처 재사용 — **137,313관측 · 484거래일**.
  > 
  > ## 결론: 새로 열리는 문은 없다
  > 
  > ### 구멍 1 — 보유 1~2일 (분봉 60분 ↔ 일봉 3일 사이, 미측정 구간)
  > 
  > | | 1일 | 2일 | 3일 | 5일 | 10일 | 20일 |
  > |---|---|---|---|---|---|---|
  > | 상위십분위 | −0.082 | −0.132 | −0.177 | −0.144 | −0.250 | −0.273 |
  > | 하위십분위 | −0.042 | −0.003 | +0.028 | +0.076 | +0.186 | +0.278 |
  > 
  > 상위는 전 구간 음수. **구멍 없음.**
  > 
  > ### 구멍 2 — 변동성 조건부 ★유망해 보였으나 기각
  > 
  > 총합 음수가 **고변동성이 끌어내린 것**임이 드러났다(일자내 ATR 3분위, 룩어헤드 없음):
  > 
  > | 분위 | ATR | 20일 상위십분위 | **양(+)인 날** | 2024/2025/2026 |
  > |---|---|---|---|---|
  > | 저변동 | 3.0% | **+0.656%** | **56%** | +1.44 / **+0.15** / +1.32 |
  > | 중변동 | 4.9% | **+0.852%** | **47%** | **−0.66** / +0.57 / +1.81 |
  > | 고변동 | 8.1% | −1.320% | 38% | −2.87 / −1.77 / −0.34 |
  > 
  > 평균만 보면 저·중변동이 비용선을 크게 넘는다. **그러나 기각한다**:
  > 
  > 1. 양(+)인 날 **47~56%** — 사전 등록 기준(**≥60%**) 미달, 사실상 동전던지기
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - [ ] 종료 상태 유지. §5 재시도 금지 목록(8항목)과 §6 재검토 조건 준수.
  - [ ] 누군가 *"저변동 종목이면 되지 않을까"* 라고 물으면 §5-1 로 답한다 — 평균이 아니라 **안정성**이 기각 사유임을 함께 전달.
  - [ ] 새 입력(C) 탐색 시에도 판정은 **평균이 아니라 [평균 + 일자 안정성 ≥60% + 연도 일관성]** 3종 세트로.

### 2026-08-23 — feat(analyze): 계정별 분석 카드 분리 + 전 종목 검색 분석 + 미로그인 실행 차단 (#386)

- **slug**: `hylee/analyze-owner-filter` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/386
- **요약**: feat(analyze): 계정별 분석 카드 분리 + 전 종목 검색 분석 + 미로그인 실행 차단
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 배경
  > 
  > `/analyze` 분석 결과 카드가 전 계정 공유였다. `ai_analysis_decisions` 가 ticker PK 라 구조적으로 계정 분리가 불가능했고, 상단 검색은 이미 로드된 목록만 필터링해 "없는 종목을 분석 시작"하는 경로가 없었다.
  > 
  > ## 변경
  > 
  > ### 1. 계정별 카드 분리
  > 
  > PK 를 `ticker` → `(ticker, requested_by)` 복합키로. 계정마다 자기 카드를 남긴다. **"최신 1건만 유지" 정책은 그대로** — 같은 계정이 재분석하면 그 계정 행만 덮어쓴다.
  > 
  > 노출 규칙 2분기:
  > - 로그인 → `requested_by = 내 계정` 행만
  > - 미로그인 → 세션 없이 저장된 분석(`''`) + 데모 종목(삼성전자·SK하이닉스·NAVER)
  > 
  > 소유자 배선:
  > - `enqueue` · 분석 handler 가 세션 이메일 기록. 워커 요청엔 쿠키가 없어 `startProcessing` 이 queue 행에서 소유자를 **서버 조회**한다(요청 body 불신).
  > - 목록 RPC · 인플라이트 큐 · 테제 무효화 배지 · 저장 결론 조회 · 삭제 · 종목명 백필을 모두 같은 뷰어 스코프로 통일.
  > - `run_id` 컬럼 추가 — 한 종목에 여러 계정 행이 생기면 "그 종목의 최신 run" 으로는 남의 분석 토큰이 내 카드에 붙는다. 행이 자기 run 을 들고 있게 한다.
  > 
  > ### 2. 전 종목 검색 → 즉시 분석
  > 
  > 상단 검색을 목록 내 필터 → `StockSearchPicker`(BFF `/api/stock/search` 재사용)로 교체. 선택 즉시 `openFor` — 카드 클릭·재분석 버튼과 같은 진입점. 목록이 비어도 검색은 남는다(첫 분석 진입점). 목록 필터링 코드는 삭제(순감).
  > 
  > ### 3. 미로그인 분석 실행 차단
  > 
  > - `requireProdSessionApi` 신설, `enqueue` 라우트에 적용(prod 한정 401). prod 의 유일한 실행 경로가 enqueue 다(직접 실행 라우트는 Vercel 에서 이미 503). 로컬 dev · 로컬 워커는 localhost 라 통과 — 개발 흐름 · 큐 드레인 무영향.
  > - prod CTA 3곳을 기존 `requested` 스위치에 `authenticated` 를 합쳐 한 번에 숨기고 "로그인하면 분석을 요청할 수 있어요" 안내로 대체. 저장된 결과 열람은 유지.
  > 
  > ## 마이그레이션
  > 
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
