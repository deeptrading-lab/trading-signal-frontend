...
### 2026-07-04 — 시장상태 인지형 마켓 홈: 장 마감 우아한 대체 + KST 폴백 + 폴링 게이팅 통합 (#247)

- **slug**: `market-status-aware-home` · **author**: @HY0118
- **PR**: https://github.com/deeptrading-lab/trading-signal-frontend/pull/247
- **요약**: 시장상태 인지형 마켓 홈: 장 마감 우아한 대체 + KST 폴백 + 폴링 게이팅 통합
- **현재 상태**: QA 통과 · 리뷰·머지 대기 (이 항목은 QA 통과 시점에 자동 기록됨)
- **PR 본문 발췌**:
  > ## 요약
  > 
  > 마켓 홈의 라이브 섹션(실시간 순위·수급 당일)이 장 마감/주말/공휴일에 띄우던 빨간·회색 에러 카드를, ②(`toss-market-calendar`)의 `useMarketStatus().isRegularOpen`·`nextOpen` 으로 게이트해 **차분한 "장 마감" 안내**로 대체한다. 호가·체결 폴링 게이트도 같은 기준으로 스왑(①/③ 후속 회수).
  > 
  > **★ 사용자 확정 결정 반영**: 기존엔 prod(TOSS 키 미설정)에서 캘린더가 null → `phase="unknown"` → `isRegularOpen=true`(fail-open)라 **마감 게이팅이 아예 발동하지 않았다**. `deriveMarketStatus` 에 **KST 휴리스틱 폴백**을 추가해 캘린더 없이도 주말/야간 마감을 잡는다(공휴일 미인지, fail-open 정신 유지).
  > 
  > ## 커밋 분할 (PRD §8)
  > 
  > 1. `feat(market)` — `deriveMarketStatus` KST 폴백 + `isKstWeekend` + 유닛 6종. 캘린더 있을 때 경로 무변경.
  > 2. `feat(home)` — `MarketClosedNotice` 공용 컴포넌트 + 실시간 순위·수급 당일 마감 게이팅 + 소프트 넛지.
  > 3. `feat(stock)` — 호가·체결 폴링 게이트 캘린더 스왑 + 패널 `closed` 라벨 정합.
  > 
  > ## AC 대응 / 자가검증
  > 
  > | AC | 결과 |
  > |---|---|
  > | AC-1 실시간 순위 마감 | `!isRegularOpen` → 4탭 `enabled` AND 로 KIS TR 무호출 + `MarketClosedNotice`. 탭바 유지 |
  > | AC-2 순위 정규장 무회귀 | `isRegularOpen=true` → 기존 4탭/failedTabs 동작 그대로 |
  > | AC-3 수급 당일 마감 | 당일+마감 → 안내 + 넛지, KIS 순매수 TR 무호출 |
  > | AC-4 수급 7일 누적 마감 | `enabled: mode==="cumulative" \|\| isRegularOpen` → 누적은 항상 조회, 안내 미노출 |
  > | AC-5 소프트 넛지 | 마감 확인 시 초기 1회 `cumulative` 착지(`settledRef` 가드, 강제 전환 아님) |
  > | AC-6 수급 정규장 무회귀 | 장중엔 당일 기본 유지, 넛지 미발동 |
  > | AC-7/9 호가·체결 폴링 | `refetchInterval` 게이트 → `isRegularOpen`. 공휴일 정지·정규장 폴링 |
  > | AC-8 폴링 fail-open | `unknown→isRegularOpen=true` → 캘린더 실패에 폴링 커플링 안 됨 |
  > | AC-10 unknown fail-open(홈) | unknown→open → 조회 정상·안내 미노출 |
  > | AC-12 컨벤션 | 신규 컴포넌트 hex/px 0, `@tanstack/react-query` 직접 import 0, 클라 `fetch(` 0, 카피 `lib/copy` 단일 |
  > | AC-13 라우트 무회귀 | `git diff main -- app/api/market app/api/flow` = 무변경(빈 출력) |
  > 
  > 검증 명령 결과:
  > - `npx tsc --noEmit` → 0 에러
- **다음 작업 후보** (PR 본문 기반, 절대적 지시 아님):
  - 나머지 폴링 소비처(`useQueryStockWarnings(Batch)`·`useQueryMinuteChart`·단타 `useIntradayPaperRefresh`·서버 스케줄러) 캘린더 스왑 — 회귀면 커 별도 후속 일괄(본 PR 이 레퍼런스).
