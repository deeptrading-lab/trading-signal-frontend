...
describe("middleware (app password gate)", () => {
  beforeEach(() => {
  ...
  describe("[AC-13] 게이트 비활성 (APP_PASSWORD 미설정)", () => {
    it("쿠키 없이도 페이지/API 전부 통과", async () => {
      ...
      const page = await middleware(makeRequest("/dashboard"));
      const api = await middleware(makeRequest("/api/market/ticker"));
  ...
  describe("[AC-2] 미인증 페이지 → /login?next=<원경로> 307", () => {
    ...
    it("쿼리 포함 원경로 보존 — /profile/AAPL?tab=x", async () => {
      const res = await middleware(makeRequest("/profile/AAPL?tab=x"));
      const location = res.headers.get("location") ?? "";
      const url = new URL(location, "http://localhost");
  ...
  describe("[AC-3] 미인증 /api/* → 401 JSON (리다이렉트 X)", () => {
    it("쿠키 없음 + /api/market/ticker → 401 unauthorized", async () => {
      const res = await middleware(makeRequest("/api/market/ticker"));
    ...
    it("쿠키 없음 + /api/workbench/analyze → 401", async () => {
      const res = await middleware(makeRequest("/api/workbench/analyze"));
      expect(res.status).toBe(401);
    });
  });
  ...
  describe("[AC-4] 예외 경로 — 항상 통과 (루프 가드)", () => {
    ...
    it("[루프 가드] 미인증 + /login → 다시 /login 리다이렉트하지 않음", async () => {
      const res = await middleware(makeRequest("/login?next=/dashboard"));
      expect(res.status).toBe(200);
      expect(res.headers.get("location")).toBeNull();
    });
  });
  ...
  describe("[AC-6] 유효 쿠키 → 통과", () => {
    it("유효 세션 쿠키 + /dashboard → 통과", async () => {
      ...
      const res = await middleware(makeRequest("/dashboard", { cookie: token }));
      expect(res.status).toBe(200);
      expect(res.headers.get("location")).toBeNull();
    });
